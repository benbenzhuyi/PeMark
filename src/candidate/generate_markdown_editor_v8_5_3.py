...
try:
    ...
    # V8.5.3 早期只有 Outline 注入；保留旧开关名作为别名。
...
TEXT_RVA = 0x1000
# V8.5.3 candidate：在 V8.5.1 基线上建立 document revision 所有权。
...
_VERSION_LABEL = 'V8.5.3' if _RELEASE_CHANNEL else 'V8.5.3 Candidate'
if ARENA_ALLOC_INJECTION_MODE != 'release':
    _window_title = ('PeMark x64 V8.5.3 ARENA-ALLOC TEST [%s]' %
                     ARENA_ALLOC_INJECTION_MODE)
elif OPEN_READ_INJECTION_MODE != 'release':
    _window_title = 'PeMark x64 V8.5.3 OPEN-READ TEST [%s]' % OPEN_READ_INJECTION_MODE
elif OPEN_TEST_BUILD:
    _window_title = 'PeMark x64 V8.5.3 OPEN-TRANSACTION TEST'
elif WRITE_INJECTION_MODE != 'release':
    _window_title = 'PeMark x64 V8.5.3 WRITE-INJECTION [%s]' % WRITE_INJECTION_MODE
else:
...
# V8.5.3 Phase E：三个样式表（start/end/type）合并为一个动态 arena 的三个区段。
...
# V8.5.3 Phase E：渲染侧派生缓冲区（位置映射 + 渲染文本）迁入动态 arena。
...
# V8.5.3 Phase E：三张样式表由动态 arena 提供，写入一律经 arena 指针。
...
# V8.5.3 Phase E：arena 未建立（分配失败或尚未预留）时不得解引用样式指针。
...
# V8.5.3 Phase E：样式表同样在扫描前按文档长度预留；分配失败放弃本次渲染，
...
# V8.5.3 Phase E：固定 131072 项样式表已删除，三个表必须经动态 arena 指针访问。
assert all(bss_sizes[name] == 8 for name in
...
# V8.5.3 Phase E：渲染侧缓冲区（位置映射 + 渲染文本）必须经动态 arena 指针访问。
...
_output_name = (('pemark_x64_v8_5_3_outline_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                if OUTLINE_ALLOC_INJECTED else
                ('pemark_x64_v8_5_3_style_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                ...
                else ('pemark_x64_v8_5_3_write_%s.exe' % WRITE_INJECTION_MODE
                      if INJECTED_BUILD else
                      'pemark_x64_v8_5_3.exe' if _RELEASE_CHANNEL else
                      'pemark_x64_v8_5_3_candidate.exe'))
...
_map_name = 'PE_MEMORY_MAP_V8_5_3.md' if _RELEASE_CHANNEL else 'PE_MEMORY_MAP_V8_5_3_CANDIDATE.md'
...
if not INJECTED_BUILD:
 with open(_map_path, 'w', encoding='utf-8') as _mf:
    ...
    _mf.write('布局（V8.5.0-pre 扩展）：text @ 0x1000（预算 0x%x 字节）、'
              ...
              % (RDATA_RVA - TEXT_RVA, RDATA_RVA, IDATA_RVA, BSS_RVA))
