...
try:
    ...
    # V8.5.4 早期只有 Outline 注入；保留旧开关名作为别名。
...
_VERSION_LABEL = 'V8.5.4' if _RELEASE_CHANNEL else 'V8.5.4 Candidate'
if ARENA_ALLOC_INJECTION_MODE != 'release':
    _window_title = ('PeMark x64 V8.5.4 ARENA-ALLOC TEST [%s]' %
                     ARENA_ALLOC_INJECTION_MODE)
elif OPEN_READ_INJECTION_MODE != 'release':
    _window_title = 'PeMark x64 V8.5.4 OPEN-READ TEST [%s]' % OPEN_READ_INJECTION_MODE
elif OPEN_TEST_BUILD:
    _window_title = 'PeMark x64 V8.5.4 OPEN-TRANSACTION TEST'
elif WRITE_INJECTION_MODE != 'release':
    _window_title = 'PeMark x64 V8.5.4 WRITE-INJECTION [%s]' % WRITE_INJECTION_MODE
else:
...
# V8.5.4 Phase E：三个样式表（start/end/type）合并为一个动态 arena 的三个区段。
...
# V8.5.4 Phase E：渲染侧派生缓冲区（位置映射 + 渲染文本）迁入动态 arena。
...
# V8.5.4：新建文档同样需要文档 arena；失败时不执行 New，保留当前文档。
...
# V8.5.4：解码与编码 scratch 现在是按需 arena；读取前必须保证两个缓冲区
...
# legacy code-page fallback is deliberately outside the V8.5.4 contract.
...
# V8.5.4：serialize 写 widebuf、编码写 bytebuf，先保证两个 arena 都能容纳本次输出。
...
# Use floor(content_h/30): a partially visible row cannot increase the valid top index.
...
# V8.5.4：先把文档 arena 提交到本次编辑器长度的容量，再读取文本。分配失败时
...
# V8.5.4 Phase E：三张样式表由动态 arena 提供，写入一律经 arena 指针。
...
# V8.5.4 Phase E：解码/编码 scratch arena。两者结构相同——容量按 64 KiB 块向上
...
# V8.5.4 Phase E：arena 未建立（分配失败或尚未预留）时不得解引用样式指针。
...
# V8.5.4 Phase E：样式表同样在扫描前按文档长度预留；分配失败放弃本次渲染，
...
# V8.5.4：搜索快照写入 widebuf，先保证 arena 有容量；失败即视为未找到。
...
assert _geo_owners <= {'resize_children', 'resize_content', 'resize_doc',
                       'resize_preview_surface', 'repaint_splitter_surface'}, \
    f'架构违规：布局几何出现在 LayoutManager 外的例程 {_geo_owners - {"resize_children", "resize_content", "resize_doc", "resize_preview_surface", "repaint_splitter_surface"}}'
...
# V8.5.4 Phase E：固定 131072 项样式表已删除，三个表必须经动态 arena 指针访问。
assert all(bss_sizes[name] == 8 for name in
...
# V8.5.4 Phase E：渲染侧缓冲区（位置映射 + 渲染文本）必须经动态 arena 指针访问。
...
# V8.5.4 Phase E：document 文本必须经保留区 + 分块提交访问。
...
# V8.5.4 Phase E：解码/编码 scratch 也必须经按需 arena 访问。
...
# V8.5.4：文件头需要容纳多个节头，因此头区扩大到 0x400（仍是 FileAlignment 的倍数）。
headers_size = 0x400
...
# V8.5.4：同一份文件布局现在用四个独立节描述，权限按内容分离，
# 而不是把它们并进一个可读可写可执行节。
...
# V8.5.4：可执行代码与数据分离。.text 只读可执行，.rdata 只读，.idata 读写
...
# (P) V8.5.4 节权限分离断言（防回退门禁）。本断言要永久拦截的错误模式：
...
_output_name = (('pemark_x64_v8_5_4_outline_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                ...
                ('pemark_x64_v8_5_4_style_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                if STYLE_ALLOC_INJECTED else
                ('pemark_x64_v8_5_4_render_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                if RENDER_ALLOC_INJECTED else
                ('pemark_x64_v8_5_4_document_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                if DOCUMENT_ALLOC_INJECTED else
                ('pemark_x64_v8_5_4_wide_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                if WIDE_ALLOC_INJECTED else
                ('pemark_x64_v8_5_4_byte_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                if BYTE_ALLOC_INJECTED else
                ('pemark_x64_v8_5_4_open_read_%s.exe' % OPEN_READ_INJECTION_MODE)
                if OPEN_READ_INJECTION_MODE != 'release' else
                'pemark_x64_v8_5_4_open_transaction_test.exe' if OPEN_TEST_BUILD
                else ('pemark_x64_v8_5_4_write_%s.exe' % WRITE_INJECTION_MODE
                      if INJECTED_BUILD else
                      'pemark_x64_v8_5_4.exe' if _RELEASE_CHANNEL else
                      'pemark_x64_v8_5_4_candidate.exe'))
...
_map_name = 'PE_MEMORY_MAP_V8_5_4.md' if _RELEASE_CHANNEL else 'PE_MEMORY_MAP_V8_5_4_CANDIDATE.md'
