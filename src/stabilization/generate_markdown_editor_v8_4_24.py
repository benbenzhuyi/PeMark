...
FILE_ALIGN = 0x200
SECT_ALIGN = 0x1000
STYLE_CAP = 131072  # V8.4.24: large-doc Markdown style span capacity
...
wstr('title','Direct PE Markdown Editor x64 V8.4.24 — No Compiler')
wstr('empty','')
wstr('menu_file','&File')
wstr('menu_edit','&Edit')
...
wstr('about','Direct PE Markdown Editor x64 V8.4.24 - stabilization candidate\r\n\r\nNative PE32+ and x86-64 machine code generated directly, without a compiler, assembler, or linker.\r\n\r\nSingle Source/Preview mode, source-offset Outline navigation, shared scrollbar geometry, and viewport-limited Preview formatting. See CHANGELOG_UNRELEASED.md for validation and remaining issues.')
...
# Capture/restore scratch HWND, never a view-mode authority.
bss_alloc('view_hwnd', 8, 8)
bss_alloc('nav_source_offset', 4, 4)
bss_alloc('nav_target_offset', 4, 4)
...
# OutlineController owns this state. Geometry fields are written only by scroll_layout.
...
imports = {
    ...
    'USER32.dll': [
        'CreateWindowExW','GetMessageW','TranslateMessage','DispatchMessageW','IsWindow','IsWindowVisible',
...
em.mov_ripmem_imm32(bsyms['view_mode'],0)
...
# V8.4.24: reserve a permanent scrollbar gutter whose width follows the current
...
# V8.4.24: register a dedicated scrollbar-surface child class.  Unlike an
...
# Left Markdown outline. V8.4.24 intentionally omits WS_VSCROLL: the ListBox
...
# V8.4.24 architecture preview: dedicated custom scrollbar surface.
...
em.call_label('capture_message_point'); em.test32('rax'); em.jcc(0x84,'dispatch')
...
# Hit-test thumb [thumb_top, thumb_top+thumb_h].
...
# Only the painted thumb starts a drag; blank horizontal gutter is not a fake thumb.
...
em.label('cmd_new')
em.xor32('rcx'); em.call_label('set_view_mode')
...
# The controller owns SOURCE visibility and menu state on a successful dialog selection.
em.xor32('rcx'); em.call_label('set_view_mode')
...
em.xor32('rcx'); em.call_iat('GetModuleHandleW'); em.mov_mrsp_reg64(0x50,'rax')
em.xor32('rcx'); em.lea_rip('rdx',rsyms['class_edit']); em.lea_rip('r8',rsyms['empty'])
...
em.mov_mrsp_imm32(0x20,0); em.mov_mrsp_imm32(0x28,0); em.mov_mrsp_imm32(0x30,100); em.mov_mrsp_imm32(0x38,100)
em.mov_mrsp_reg64(0x40,'rbx'); em.mov_mrsp_imm32(0x48,1,qword=True); em.mov_mrsp_imm32(0x58,0,qword=True)
em.call_iat('CreateWindowExW'); em.mov_r64_r64('rsi','rax'); em.mov_ripmem_r64(bsyms['hwnd_edit'],'rsi'); em.test64('rax'); em.jcc(0x84,'exit')
em.mov_r64_r64('rcx','rsi'); em.mov_r32_imm('rdx',0x00C5); em.mov_r32_imm('r8',WIDE_CHARS-1); em.xor32('r9'); em.call_iat('SendMessageW')
em.mov_r64_r64('rcx','rsi'); em.lea_rip('rdx',bsyms['document_model']); em.call_iat('SetWindowTextW')
...
# Source control recreation does not change Markdown or the Preview surface.
em.call_label('resize_children'); em.call_label('rebuild_outline'); em.call_label('refresh_preview_visible_theme'); em.call_label('update_status')
em.call_label('apply_view_visibility'); em.call_label('active_view_hwnd'); em.mov_r64_r64('rcx','rax'); em.call_iat('SetFocus'); em.jmp('msg_loop')
...
em.mov_r32_ripmem('rax',bsyms['view_mode']); em.test32('rax'); em.jcc(0x84,'preview_show')
...
# Build hidden Preview without changing the committed view mode.
em.call_label('build_preview')
...
# Restore the hidden destination, then commit the mode once.
em.call_label('resize_children')
em.mov_r64_ripmem('rcx',bsyms['hwnd_preview']); em.call_label('restore_surface_state')
em.mov_r32_imm('rcx',1); em.call_label('set_view_mode')
...
em.mov_r32_ripmem('rax',bsyms['view_mode']); em.test32('rax'); em.jcc(0x84,'outline_capture_source')
...
em.mov_r64_ripmem('rcx',bsyms['hwnd_main']); em.mov_r32_imm('rdx',0x4D); em.call_iat('KillTimer')
em.mov_r32_ripmem('rax',bsyms['view_mode']); em.test32('rax'); em.jcc(0x84,'theme_fast_source')
...
em.jmp('exit')  # Destroyed main window must never fall through into a helper.

# ------------------------------------------------------------------
# Helper: synchronize independent Outline scrollbar range/position.
# ---------------- ViewController ----------------
# SOURCE=0/PREVIEW=1. Only set_view_mode mutates the mode after zero-initialization.
# view_hwnd is capture/restore scratch; active_view_hwnd derives the destination.
em.label('active_view_hwnd')
em.mov_r32_ripmem('rax',bsyms['view_mode']); em.test32('rax'); em.jcc(0x84,'active_view_source')
em.mov_r64_ripmem('rax',bsyms['hwnd_preview']); em.emit(0xC3)
em.label('active_view_source'); em.mov_r64_ripmem('rax',bsyms['hwnd_edit']); em.emit(0xC3)

# rcx = mode. Preserves nonvolatile registers; owns visibility/menu/timer commit.
em.label('set_view_mode')
em.emit(0x48,0x83,0xEC,0x28)
em.mov_ripmem_r32(bsyms['view_mode'],'rcx')
em.call_label('apply_view_visibility')
em.mov_r32_ripmem('rax',bsyms['view_mode']); em.test32('rax'); em.jcc(0x85,'view_commit_done')
em.mov_r64_ripmem('rcx',bsyms['hwnd_main']); em.mov_r32_imm('rdx',0x4D); em.call_iat('KillTimer')
em.mov_ripmem_imm32(bsyms['preview_theme_dirty'],0)
em.label('view_commit_done'); em.add_r64_imm8('rsp',0x28); em.emit(0xC3)

# Temporary load transaction: no new state flag; caller must restore visibility.
em.label('suspend_view_surfaces')
em.emit(0x48,0x83,0xEC,0x28)
...
em.label('apply_view_visibility')
em.emit(0x48,0x83,0xEC,0x28)
em.mov_r32_ripmem('rax',bsyms['view_mode']); em.test32('rax'); em.jcc(0x84,'view_apply_source')
em.mov_r64_ripmem('rcx',bsyms['hwnd_edit']); em.xor32('rdx'); em.call_iat('ShowWindow')
em.mov_r64_ripmem('rcx',bsyms['hwnd_preview']); em.mov_r32_imm('rdx',5); em.call_iat('ShowWindow')
em.mov_r32_imm('r8',8); em.jmp('view_apply_menu')
em.label('view_apply_source')
em.mov_r64_ripmem('rcx',bsyms['hwnd_preview']); em.xor32('rdx'); em.call_iat('ShowWindow')
em.mov_r64_ripmem('rcx',bsyms['hwnd_edit']); em.mov_r32_imm('rdx',5); em.call_iat('ShowWindow')
em.xor32('r8')
em.label('view_apply_menu')
em.mov_r64_ripmem('rcx',bsyms['hmenu_view']); em.mov_r32_imm('rdx',1306); em.call_iat('CheckMenuItem')
em.add_r64_imm8('rsp',0x28); em.emit(0xC3)
...
# Enforce the same maximum as drag/page/wheel, including after native ListBox navigation.
...
# Inputs: count/top, content_h, scrollbar_w. Outputs: cached track/thumb/travel.
# Only MulDiv is called, with all live values in named state. No paint or HWND mutation.
em.label('scroll_layout')
em.emit(0x48,0x83,0xEC,0x28)
# floor(content_h / 30), not MulDiv's round-to-nearest: partially visible rows do not count.
em.mov_r32_ripmem('rax',bsyms['content_h']); em.test32('rax'); em.jcc(0x89,'scroll_height_ok'); em.xor32('rax')
...
em.mov_r32_ripmem('r10',bsyms['content_h']); em.cmp_r32_imm('r10',8); em.jcc(0x8F,'sync_track_ok'); em.mov_r32_imm('r10',8)
em.label('sync_track_ok'); em.sub_r32_imm8('r10',8); em.mov_ripmem_r32(bsyms['outline_scroll_track_h'],'r10')
# thumb_h = track if all rows fit, otherwise max(24, visibleRows*track/count)
em.mov_r32_ripmem('r11',bsyms['outline_scroll_count']); em.test32('r11'); em.jcc(0x84,'sync_thumb_full'); em.mov_r32_ripmem('rax',bsyms['outline_visible_rows']); em.cmp_r32_r32('rax','r11'); em.jcc(0x82,'sync_thumb_calc')
...
# Cache rectangles in scrollbar-local coordinates. Painter and hit-test read this exact thumb.
...
# visual width clamp(gutter/2, 6..9); never wider than gutter.
...
# Reveal zone starts 6px before gutter and extends to 4px past divider.
...
em.mov_r32_ripmem('r11',bsyms['outline_width']); em.add_r32_imm8('r11',4); em.cmp_r32_r32('r10','r11'); em.jcc(0x8F,'hover_hide_custom')
...
# MSG.pt records the pointer at event generation. GetCursorPos reads the later
# position and can turn a fast queued drag into a track click. Preserve event time.
...
em.mov_r32_ripmem('r11',bsyms['outline_scroll_travel']); em.cmp_r32_r32('r10','r11'); em.jcc(0x86,'os_drag_pos_ok'); em.mov_r32_r32('r10','r11')
...
em.mov_r32_ripmem('r10',bsyms['cursor_pt']); em.cmp_r32_imm('r10',140); em.jcc(0x8D,'split_drag_min_ok'); em.mov_r32_imm('r10',140)
...
# Actual viewport endpoint plus a small prefetch margin. The former fixed 40000
# code-unit window caused multi-second input stalls even on a 200KB document.
# EM_CHARFROMPOS RichEdit takes a POINTL pointer, returns a 32-bit character index.
...
em.label('visible_window_fallback'); em.mov_r32_ripmem('rax',bsyms['view_top_pos']); em.mov_r32_imm('r11',2000); em.add_r32_r32('rax','r11')
...
em.emit(0x48,0x83,0xEC,0x28)
em.mov_r32_ripmem('rax',bsyms['view_mode']); em.test32('rax'); em.jcc(0x84,'refresh_visible_ret')
...
# Outline ListBox content only; it can never paint into the scrollbar gutter.
em.mov_r64_ripmem('rcx',bsyms['hwnd_outline']); em.xor32('rdx'); em.xor32('r8'); em.mov_r32_r32('r9','rax'); em.mov_r32_ripmem('r11',bsyms['content_h']); em.mov_mrsp_reg32(0x20,'r11'); em.mov_mrsp_imm32(0x28,1,qword=True); em.call_iat('MoveWindow')
# Gutter background at x=listboxWidth, width=scrollbar_w.
em.mov_r64_ripmem('rcx',bsyms['hwnd_outline_gutter']); em.mov_r32_ripmem('rdx',bsyms['outline_width']); em.mov_r32_ripmem('rax',bsyms['scrollbar_w']); em.sub_r32_r32('rdx','rax'); em.xor32('r8'); em.mov_r32_r32('r9','rax'); em.mov_r32_ripmem('r11',bsyms['content_h']); em.mov_mrsp_reg32(0x20,'r11'); em.mov_mrsp_imm32(0x28,1,qword=True); em.call_iat('MoveWindow')
# Independent scrollbar occupies exactly the same gutter rectangle.
em.mov_r64_ripmem('rcx',bsyms['hwnd_outline_scroll']); em.mov_r32_ripmem('rdx',bsyms['outline_width']); em.mov_r32_ripmem('rax',bsyms['scrollbar_w']); em.sub_r32_r32('rdx','rax'); em.xor32('r8'); em.mov_r32_r32('r9','rax'); em.mov_r32_ripmem('r11',bsyms['content_h']); em.mov_mrsp_reg32(0x20,'r11'); em.mov_mrsp_imm32(0x28,1,qword=True); em.call_iat('MoveWindow')
...
em.emit(0x48,0x83,0xEC,0x28)
em.mov_r32_ripmem('rax',bsyms['view_mode']); em.test32('rax'); em.jcc(0x84,'zoom_anchor_source')
...
em.call_label('suspend_view_surfaces')
...
em.mov_r64_ripmem('rcx',bsyms['hwnd_outline']); em.xor32('rdx'); em.mov_r32_imm('r8',1); em.call_iat('InvalidateRect'); em.mov_r64_ripmem('rcx',bsyms['hwnd_outline_gutter']); em.xor32('rdx'); em.mov_r32_imm('r8',1); em.call_iat('InvalidateRect'); em.call_label('sync_outline_scrollbar')
...
# A single huge code block must not make viewport styling format the entire document.
...
# Event-driven refresh: always sync Outline, render only in committed Preview mode.
em.label('update_preview')
em.emit(0x48,0x83,0xEC,0x28)
em.call_label('rebuild_outline')
em.mov_r32_ripmem('rax',bsyms['view_mode']); em.test32('rax'); em.jcc(0x84,'update_preview_done')
em.call_label('build_preview')
em.label('update_preview_done'); em.add_r64_imm8('rsp',0x28); em.emit(0xC3)

em.label('build_preview')
...
# RenderModel is built from DocumentModel; caller decides when to show it.
...
em.test32('r15'); em.jcc(0x84,'pv8_outside_fence')
# The marker probe must not consume indentation or run again inside this line.
em.mov_r32_ripmem('r12',bsyms['line_src_start']); em.xor32('r14'); em.jmp('pv8_codeblock_copy')
em.label('pv8_outside_fence')
...
em.mov_r64_ripmem('rcx',bsyms['hwnd_preview']); em.lea_rip('rdx',bsyms['previewbuf']); em.call_iat('SetWindowTextW')
...
em.mov_ripmem_r32(bsyms['nav_source_offset'],'r8'); em.mov_ripmem_r32(bsyms['nav_target_offset'],'r8')
em.mov_r32_ripmem('rax',bsyms['view_mode']); em.test32('rax'); em.jcc(0x84,'navigate_source')
em.call_label('map_source_to_render'); em.mov_ripmem_r32(bsyms['nav_target_offset'],'rax')
em.label('navigate_source')
em.call_label('active_view_hwnd'); em.mov_r64_r64('rcx','rax'); em.mov_ripmem_r64(bsyms['nav_hwnd'],'rax')
em.mov_r32_ripmem('r8',bsyms['nav_target_offset']); em.mov_r32_r32('r9','r8')
...
# A programmatic Outline jump can land in a never-before-formatted lazy Preview
# region. Format that viewport synchronously so headings/code/link colors are right
# on the first frame instead of only becoming correct after the user scrolls.
em.mov_r32_ripmem('rax',bsyms['view_mode']); em.test32('rax'); em.jcc(0x84,'navigate_ret'); em.call_label('refresh_preview_visible_theme')
...
# Cached immutable geometry from OutlineController.
...
em.label('sp_fill_thumb'); em.mov_r64_ripmem('rcx',bsyms['paint_hdc']); em.lea_rip('rdx',bsyms['outline_thumb_rect']); em.call_iat('FillRect')
...
em.cmp_r32_imm('rdx',0x0215); em.jcc(0x84,'wp_capture_lost')
em.cmp_r32_imm('rdx',0x001F); em.jcc(0x84,'wp_cancel_mode')
...
em.label('wp_cancel_mode'); em.call_iat('ReleaseCapture')
em.label('wp_capture_lost')
em.mov_ripmem_imm32(bsyms['outline_scroll_drag'],0); em.mov_ripmem_imm32(bsyms['splitter_drag'],0)
...
em.mov_r32_mreg('rax','r9',4); em.cmp_r32_imm('rax',2); em.jcc(0x84,'wp_draw_status'); em.cmp_r32_imm('rax',4); em.jcc(0x84,'wp_draw_outline'); em.cmp_r32_imm('rax',5); em.jcc(0x84,'wp_draw_corner'); em.cmp_r32_imm('rax',6); em.jcc(0x84,'wp_draw_splitter'); em.cmp_r32_imm('rax',7); em.jcc(0x84,'wp_draw_gutter'); # ID 8 is now a dedicated custom child window; it no longer uses WM_DRAWITEM. em.jmp('wp_default')
...
# V8.4.24: hard-isolate owner-drawn Outline rows from the native scrollbar gutter.
...
out = str(__import__('pathlib').Path(__file__).resolve().parents[2] / 'bin/stabilization/direct_pe_markdown_editor_x64_v8_4_24.exe')
