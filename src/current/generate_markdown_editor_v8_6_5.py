...
# TERMINAL_SERVER_AWARE 确保 Windows 在 RDP 会话中正确初始化进程环境。
...
_VERSION_LABEL = 'V8.6.5' if _RELEASE_CHANNEL else 'V8.6.5 Candidate'
...
wstr('m_right_sidebar','Right &Sidebar\tCtrl+J')
...
# V8.6.1：新式"选择文件夹"走 IFileOpenDialog，与 GetOpenFileNameW 同属通用项
# 对话框家族，因此外观/键盘/主题与"打开文件"一致；旧式 SHBrowseForFolderW 只在
# CoCreateInstance 失败时回落。COM 的 GUID 必须原样落在 .rdata。
# {DC1C5A9C-E88A-4dde-A5A1-60F82A20AEF7}
add_bytes('clsid_fileopendialog',
          u32(0xDC1C5A9C) + u16(0xE88A) + u16(0x4DDE) +
          bytes((0xA5,0xA1,0x60,0xF8,0x2A,0x20,0xAE,0xF7)), 4)
# {d57c7288-d4ad-4768-be02-9d969532d960}
add_bytes('iid_ifileopen',
          u32(0xD57C7288) + u16(0xD4AD) + u16(0x4768) +
          bytes((0xBE,0x02,0x9D,0x96,0x95,0x32,0xD9,0x60)), 4)
...
_accels = [
    ...
    (FVIRTKEY|FCONTROL, 0x4A, 1315),                  # Ctrl+J Right Sidebar (reserved)
...
# V8.6.1：文件面板的滚轮分流（滚动条本身由 ListBox 原生绘制；这三个字段只
# 用于把"指针在哪个面板上滚"换算成 LB_SETTOPINDEX，并给测试提供读数）。
...
bss_alloc('files_scroll_hot', 4, 4)
...
# tree_expand_children inserts into the dynamic tree arena while it enumerates.
# Arena growth can move that storage, so the parent path must not remain a
# pointer into a tree row across insertions.
bss_alloc('tree_expand_path_buf', 512*2, 2)
...
# V8.6.4 收尾：内存内最近文件列表（最多 10 条，每条 512 UTF-16 单元，与
# tree_expanded 的路径缓冲同宽）。File 菜单底部由 rebuild_recent_menu 动态
# 重建；file_menu_base_count 记录静态构建后的项数，作为删除尾差的基准。
bss_alloc('recent_count', 4, 4)
bss_alloc('recent_paths', 10*512*2, 2)
bss_alloc('file_menu_base_count', 4, 4)
# V8.6.4 预留：V9 AI 右侧栏的显隐状态。V8.6 布局不消费它；View 菜单的
# Right Sidebar 命令（1315/Ctrl+J）只翻转状态并同步勾选，为 AI 侧栏占住
# 菜单入口与快捷键。
bss_alloc('right_sidebar_visible', 4, 4)
bss_alloc('ws_path_buf', 512*2, 16)
# BROWSEINFOW（x64 共 64 字节）：pszDisplayName 复用 ws_path_buf。
bss_alloc('browseinfo', 64, 8)
# V8.6.1 新式文件夹选择（IFileOpenDialog）：指针、选项位与取回路径的落点。
bss_alloc('fod_ptr', 8, 8)
bss_alloc('fod_options', 4, 4)
bss_alloc('fod_chosen', 4, 4)
bss_alloc('shell_item', 8, 8)
bss_alloc('item_path', 8, 8)
bss_alloc('fod_probe_coinit', 4, 4)
bss_alloc('fod_probe_hresult', 4, 4)
bss_alloc('fod_probe_options_hresult', 4, 4)
bss_alloc('fod_probe_vtbl', 8, 8)
bss_alloc('fod_probe_entered', 4, 4)
...
imports = {
    ...
    'USER32.dll': [
        ...
        'TrackPopupMenu','IsZoomed','TrackMouseEvent','GetMenuItemCount','RemoveMenu',
...
append_imm('r14',0,1306,'m_preview'); append_imm('r14',0x8,1307,'m_outline'); append_imm('r14',0,1315,'m_right_sidebar')
# V8.6.4：面板菜单项改为互斥的最大化开关——勾选 = 该面板 maximized 且另一
# 面板 minimized。默认 half/half 时两项都不勾选；勾选始终由 sync_panel_menu
# 按 files_state/outline_state 同步，菜单创建时不需要预置。
append_sep('r14'); append_imm('r14',0,1308,'m_panel_files'); append_imm('r14',0,1309,'m_panel_outline')
...
# V8.6.4：记录 File 菜单静态命令段（New..Close）的项数。静态构建的尾部
# sep + Exit 与动态段一起归 rebuild_recent_menu 重建，因此基准 = 总项数 - 2；
# 否则首次重建会在旧 Exit 之后再追加一整段，Exit 出现两次。
em.mov_r64_ripmem('rcx',bsyms['hmenu_file']); em.call_iat('GetMenuItemCount'); em.sub_r32_imm8('rax',2); em.mov_ripmem_r32(bsyms['file_menu_base_count'],'rax')
...
# V8.6.1：两个面板同时存在，滚轮按"指针落在哪个面板"分流；先看文件面板。
# 文件列表用的是原生滚动条，手动分流只是让它不必先获得焦点也能滚。
...
em.mov_r64_ripmem('rcx',bsyms['hwnd_files']); em.test64('rcx'); em.jcc(0x84,'wheel_not_files')
em.lea_rip('rdx',bsyms['rect']); em.call_iat('GetWindowRect'); em.test32('rax'); em.jcc(0x84,'wheel_not_files')
em.mov_r32_ripmem('r10',bsyms['wheel_x']); em.mov_r32_ripmem('r11',bsyms['rect']); em.cmp_r32_r32('r10','r11'); em.jcc(0x8C,'wheel_not_files'); em.mov_r32_ripmem('r11',bsyms['rect']+8); em.cmp_r32_r32('r10','r11'); em.jcc(0x8D,'wheel_not_files')
em.mov_r32_ripmem('r10',bsyms['wheel_y']); em.mov_r32_ripmem('r11',bsyms['rect']+4); em.cmp_r32_r32('r10','r11'); em.jcc(0x8C,'wheel_not_files'); em.mov_r32_ripmem('r11',bsyms['rect']+12); em.cmp_r32_r32('r10','r11'); em.jcc(0x8D,'wheel_not_files')
em.call_label('wheel_scroll_files'); em.jmp('msg_loop')
em.label('wheel_not_files')
...
_command_routes = [(1001,'cmd_new'),(1002,'cmd_open'),(1003,'cmd_save'),(1004,'cmd_saveas'),(1005,'cmd_exit'),
                  ...
                  (1315,'cmd_right_sidebar'),
                  (1316,'cmd_recent_0'),(1317,'cmd_recent_1'),(1318,'cmd_recent_2'),(1319,'cmd_recent_3'),(1320,'cmd_recent_4'),
                  (1321,'cmd_recent_5'),(1322,'cmd_recent_6'),(1323,'cmd_recent_7'),(1324,'cmd_recent_8'),(1325,'cmd_recent_9'),
...
if OPEN_TEST_BUILD:
    ...
    _command_routes.append((1909, 'cmd_folder_com_probe'))
...
# 从消息泵 jmp 进入（不是 call），因此帧大小必须是 16 的倍数：否则调用 API 时
# 栈未对齐，COM 激活路径（CoCreateInstance）会直接失败。
em.emit(0x48,0x83,0xEC,0x30)
# 选择工作区目录。V8.6.1：优先用新式通用项对话框（IFileOpenDialog + FOS_PICKFOLDERS），
# 它的外观、键盘与主题和"打开文件"的 GetOpenFileNameW 完全一致；只有 COM 创建失败时
# 才回落到旧式 SHBrowseForFolderW（那是系统里另一套老 UI，正是两边风格不一致的来源）。
# 通用对话框走 shell COM，先在调用线程初始化（失败也无妨）。
em.xor32('rcx'); em.mov_r32_imm('rdx',2); em.call_iat('CoInitializeEx')
em.mov_ripmem_imm32(bsyms['fod_chosen'],0)
em.lea_rip('rcx',rsyms['clsid_fileopendialog']); em.xor32('rdx'); em.mov_r32_imm('r8',1)
em.lea_rip('r9',rsyms['iid_ifileopen']); em.lea_rip('rax',bsyms['fod_ptr']); em.mov_mrsp_reg64(0x20,'rax'); em.call_iat('CoCreateInstance')
em.test32('rax'); em.jcc(0x88,'cmd_open_folder_legacy')
# GetOptions -> 叠加 FOS_PICKFOLDERS(0x20) | FOS_FORCEFILESYSTEM(0x40) -> SetOptions，
# 保留系统默认选项（PATHMUSTEXIST 等），只把"选目录"打开。
em.mov_r64_ripmem('rcx',bsyms['fod_ptr']); em.lea_rip('rdx',bsyms['fod_options']); em.mov_r64_mreg('rax','rcx',0); em.mov_r64_mreg('rax','rax',80); em.call_r64('rax')
em.mov_r32_ripmem('rdx',bsyms['fod_options']); em.or_r32_imm('rdx',0x60)
em.mov_r64_ripmem('rcx',bsyms['fod_ptr']); em.mov_r64_mreg('rax','rcx',0); em.mov_r64_mreg('rax','rax',72); em.call_r64('rax')
em.mov_r64_ripmem('rcx',bsyms['fod_ptr']); em.lea_rip('rdx',rsyms['folder_title']); em.mov_r64_mreg('rax','rcx',0); em.mov_r64_mreg('rax','rax',136); em.call_r64('rax')
# Show(hwnd_main)：用户取消会返回失败 HRESULT，直接走释放。
em.mov_r64_ripmem('rcx',bsyms['fod_ptr']); em.mov_r64_r64('rdx','rbx')
em.mov_r64_mreg('rax','rcx',0); em.mov_r64_mreg('rax','rax',24); em.call_r64('rax')
em.test32('rax'); em.jcc(0x88,'cmd_open_folder_release')
# GetResult(&shell_item) -> IShellItem::GetDisplayName(SIGDN_FILESYSPATH)。
em.mov_r64_ripmem('rcx',bsyms['fod_ptr']); em.lea_rip('rdx',bsyms['shell_item']); em.mov_r64_mreg('rax','rcx',0); em.mov_r64_mreg('rax','rax',160); em.call_r64('rax')
em.test32('rax'); em.jcc(0x88,'cmd_open_folder_release')
em.mov_r64_ripmem('rcx',bsyms['shell_item']); em.mov_r32_imm('rdx',0x80058000); em.lea_rip('r8',bsyms['item_path'])
em.mov_r64_mreg('rax','rcx',0); em.mov_r64_mreg('rax','rax',40); em.call_r64('rax')
em.test32('rax'); em.jcc(0x88,'cmd_open_folder_item_release')
em.mov_r64_ripmem('rcx',bsyms['item_path']); em.call_label('workspace_set_root')
em.mov_r64_ripmem('rcx',bsyms['item_path']); em.call_iat('CoTaskMemFree')
em.mov_ripmem_imm32(bsyms['fod_chosen'],1)
em.label('cmd_open_folder_item_release')
em.mov_r64_ripmem('rcx',bsyms['shell_item'])
em.mov_r64_mreg('rax','rcx',0); em.mov_r64_mreg('rax','rax',16); em.call_r64('rax')
em.label('cmd_open_folder_release')
em.mov_r64_ripmem('rcx',bsyms['fod_ptr']); em.mov_r64_mreg('rax','rcx',0); em.mov_r64_mreg('rax','rax',16); em.call_r64('rax')
em.jmp('cmd_open_folder_result')
em.label('cmd_open_folder_legacy')
...
em.test64('rax'); em.jcc(0x84,'cmd_open_folder_done')
...
em.test32('r14'); em.jcc(0x84,'cmd_open_folder_done')
em.lea_rip('rcx',bsyms['temp_path']); em.call_label('workspace_set_root')
em.mov_ripmem_imm32(bsyms['fod_chosen'],1)
em.label('cmd_open_folder_result')
em.mov_r32_ripmem('rax',bsyms['fod_chosen']); em.test32('rax'); em.jcc(0x84,'cmd_open_folder_done')
em.mov_r32_imm('rcx',1); em.call_label('set_panel_mode'); em.call_label('update_status')
em.label('cmd_open_folder_done'); em.add_r64_imm8('rsp',0x30); em.jmp('msg_loop')
...
# V8.6.4：打开事务提交后推进最近文件列表并重建 File 菜单尾部。push 只在
# 这里发生：New/Close 不入列，另存为不改变列表（仍是"最近打开"语义）。
em.call_label('push_recent_path'); em.call_label('rebuild_recent_menu')
em.call_label('commit_clean_document'); em.call_label('update_preview'); em.call_label('update_status'); em.jmp('msg_loop')
...
# max(0, count - visibleRows)，与大纲拖动到底时的手感一致（visibleRows = 列表高/30）。
# 文件列表用的是原生滚动条，这里只驱动 LB_SETTOPINDEX，thumb 由 ListBox 自己跟随。
em.label('wheel_scroll_files')
em.emit(0x48,0x83,0xEC,0x28)
em.mov_r64_ripmem('rcx',bsyms['hwnd_files']); em.test64('rcx'); em.jcc(0x84,'wsf_ret')
...
em.mov_r32_ripmem('rax',bsyms['files_list_h']); em.test32('rax'); em.jcc(0x89,'wsf_rows_nonnegative'); em.xor32('rax')
em.label('wsf_rows_nonnegative'); em.xor32('rdx'); em.mov_r32_imm('rcx',30); em.emit(0xF7,0xF1)
em.test32('rax'); em.jcc(0x85,'wsf_rows_min'); em.mov_r32_imm('rax',1)
em.label('wsf_rows_min'); em.mov_ripmem_r32(bsyms['files_scroll_visible_rows'],'rax')
...
em.mov_rax_mr12(16); em.mov_r32_r32('r10','rax'); em.shr_r32_imm8('r10',16); em.mov_r32_r32('r11','r10'); em.and_r32_imm('r11',0x8000); em.test32('r11'); em.jcc(0x85,'wsf_down')
em.mov_r32_ripmem('r8',bsyms['files_scroll_top']); em.cmp_r32_imm('r8',3); em.jcc(0x83,'wsf_up_sub'); em.xor32('r8'); em.jmp('wsf_send')
em.label('wsf_up_sub'); em.sub_r32_imm8('r8',3); em.jmp('wsf_send')
em.label('wsf_down')
em.mov_r32_ripmem('r8',bsyms['files_scroll_top']); em.add_r32_imm8('r8',3)
em.mov_r32_ripmem('r10',bsyms['files_scroll_count']); em.mov_r32_ripmem('r11',bsyms['files_scroll_visible_rows']); em.sub_r32_r32('r10','r11')
em.test32('r10'); em.jcc(0x89,'wsf_maxtop'); em.xor32('r10')
em.label('wsf_maxtop'); em.cmp_r32_r32('r8','r10'); em.jcc(0x86,'wsf_send'); em.mov_r32_r32('r8','r10')
em.label('wsf_send')
...
# thumb 常态 6px、指针靠近（hot）11px，都居中于槽位；用移位代替除法。
em.mov_r32_imm('r10',6)
em.mov_r32_ripmem('rax',bsyms['files_scroll_hot']); em.test32('rax'); em.jcc(0x84,'fsl_thumb_width'); em.mov_r32_imm('r10',11)
em.label('fsl_thumb_width')
em.lea_rip('rcx',bsyms['files_thumb_rect']); em.mov_r32_ripmem('r11',bsyms['scrollbar_w']); em.sub_r32_r32('r11','r10'); em.shr_r32_imm8('r11',1)
em.mov_mreg_reg32('rcx',0,'r11'); em.add_r32_r32('r11','r10'); em.mov_mreg_reg32('rcx',8,'r11')
...
# V8.6.1：thumb 常态 6px、指针靠近（hot）11px，居中于槽位；与文件面板同一套规则，
# 绘制也都是同一段 RoundRect 胶囊代码。
em.mov_r32_imm('r10',6)
em.mov_r32_ripmem('rax',bsyms['outline_scroll_hot']); em.test32('rax'); em.jcc(0x84,'sync_thumb_width'); em.mov_r32_imm('r10',11)
em.label('sync_thumb_width')
em.lea_rip('rcx',bsyms['outline_thumb_rect']); em.mov_r32_ripmem('r11',bsyms['scrollbar_w']); em.sub_r32_r32('r11','r10'); em.shr_r32_imm8('r11',1)
em.mov_mreg_reg32('rcx',0,'r11'); em.add_r32_r32('r11','r10'); em.mov_mreg_reg32('rcx',8,'r11')
...
# 指针对准本面板滚动槽位时把 thumb 变粗（与主编辑区原生滚动条一致的手感）。
em.xor32('r9')
em.mov_r32_ripmem('r10',bsyms['cursor_pt']); em.mov_r32_ripmem('r11',bsyms['outline_width']); em.mov_r32_ripmem('rax',bsyms['scrollbar_w']); em.mov_r32_r32('r8','r11'); em.sub_r32_r32('r8','rax')
em.cmp_r32_r32('r10','r8'); em.jcc(0x8C,'hover_outline_hot_done'); em.cmp_r32_r32('r10','r11'); em.jcc(0x8D,'hover_outline_hot_done')
em.mov_r32_ripmem('r10',bsyms['cursor_pt']+4); em.mov_r32_ripmem('r11',bsyms['outline_list_y']); em.cmp_r32_r32('r10','r11'); em.jcc(0x8C,'hover_outline_hot_done')
em.mov_r32_ripmem('rax',bsyms['outline_list_h']); em.add_r32_r32('r11','rax'); em.cmp_r32_r32('r10','r11'); em.jcc(0x8D,'hover_outline_hot_done')
em.mov_r32_imm('r9',1)
em.label('hover_outline_hot_done')
em.mov_r32_ripmem('rax',bsyms['outline_scroll_hot']); em.cmp_r32_r32('rax','r9'); em.jcc(0x84,'hover_outline_cursor')
em.mov_ripmem_r32(bsyms['outline_scroll_hot'],'r9'); em.call_label('sync_outline_scrollbar')
em.label('hover_outline_cursor')
...
# V8.6.1：文件面板滚动槽位的悬停判定——只负责把 thumb 变粗（常态 6px / hot 11px），
# 可见性仍由几何决定。与大纲同一套规则。
em.label('update_files_hover')
em.emit(0x48,0x83,0xEC,0x38)
em.mov_r64_ripmem('rcx',bsyms['hwnd_files_scroll']); em.test64('rcx'); em.jcc(0x84,'hover_fs_ret')
em.mov_r32_ripmem('rax',bsyms['outline_flag']); em.test32('rax'); em.jcc(0x84,'hover_fs_off')
em.lea_rip('rcx',bsyms['cursor_pt']); em.call_iat('GetCursorPos'); em.test32('rax'); em.jcc(0x84,'hover_fs_off')
em.mov_r64_ripmem('rcx',bsyms['hwnd_main']); em.lea_rip('rdx',bsyms['cursor_pt']); em.call_iat('ScreenToClient'); em.test32('rax'); em.jcc(0x84,'hover_fs_off')
em.mov_r32_ripmem('r10',bsyms['cursor_pt']); em.mov_r32_ripmem('r11',bsyms['outline_width']); em.mov_r32_ripmem('rax',bsyms['scrollbar_w']); em.mov_r32_r32('r8','r11'); em.sub_r32_r32('r8','rax')
em.cmp_r32_r32('r10','r8'); em.jcc(0x8C,'hover_fs_off'); em.cmp_r32_r32('r10','r11'); em.jcc(0x8D,'hover_fs_off')
em.mov_r32_ripmem('r10',bsyms['cursor_pt']+4); em.mov_r32_ripmem('r11',bsyms['files_list_y']); em.cmp_r32_r32('r10','r11'); em.jcc(0x8C,'hover_fs_off')
em.mov_r32_ripmem('rax',bsyms['files_list_h']); em.add_r32_r32('r11','rax'); em.cmp_r32_r32('r10','r11'); em.jcc(0x8D,'hover_fs_off')
em.mov_r32_imm('r9',1); em.jmp('hover_fs_apply')
em.label('hover_fs_off'); em.xor32('r9')
em.label('hover_fs_apply')
em.mov_r32_ripmem('rax',bsyms['files_scroll_hot']); em.cmp_r32_r32('rax','r9'); em.jcc(0x84,'hover_fs_ret')
em.mov_ripmem_r32(bsyms['files_scroll_hot'],'r9'); em.call_label('sync_files_scrollbar')
em.label('hover_fs_ret'); em.add_r64_imm8('rsp',0x38); em.emit(0xC3)
...
def _cap_round_rect_rel(rect_idx, x1, y1, x2, y2, radius):
    _cap_set_draw_rel(rect_idx, x1, y1, x2, y2)
    em.mov_r64_ripmem('rcx', bsyms['paint_hdc'])
    em.lea_rip('rdx', bsyms['draw_rect'])
    em.mov_r32_mreg('r8', 'rdx', 0)
    ...
    em.mov_r64_r64('r9', 'rax')
    em.mov_mrsp_reg32(0x20, 'r10')
    em.mov_mrsp_imm32(0x28, radius)
    em.mov_mrsp_imm32(0x30, radius)
    em.call_iat('RoundRect')
...
em.call_iat('SelectObject')
# Codex-style panel toggles: rounded rectangle + vertical divider. These two
# are deliberately drawn rather than taken from the icon font because the MDL2
# pane glyphs are arrows and do not match the title-row language.
_cap_hover_bg('toggle', CAP_ID['toggle'], CAP_RECT['toggle'])
_cap_use_pen()
_cap_round_rect_rel(CAP_RECT['toggle'], 4, 5, 20, 19, 3)
_cap_line_rel(CAP_RECT['toggle'], 10, 5, 10, 19)
...
em.label('cap_glyph_code')
_cap_use_pen()
_cap_line_rel(CAP_RECT['viewmode'], 9, 6, 4, 12)
_cap_line_rel(CAP_RECT['viewmode'], 4, 12, 9, 18)
_cap_line_rel(CAP_RECT['viewmode'], 15, 6, 20, 12)
_cap_line_rel(CAP_RECT['viewmode'], 20, 12, 15, 18)
_cap_line_rel(CAP_RECT['viewmode'], 13, 4, 11, 20)
...
_cap_hover_bg('rightbar', CAP_ID['rightbar'], CAP_RECT['rightbar'])
_cap_use_pen()
...
em.mov_r32_ripmem('rax',bsyms['zoom_pct']); em.emit(0x6B,0xC0,0x11); em.emit(0x99); em.mov_r32_imm('rcx',100); em.emit(0xF7,0xF9); em.emit(0xF7,0xD8)
...
# 文件树模型从同一个授权根重建；rebuild_file_list 是唯一的树投影写者。
...
# rcx = destination path slot, rdx = source path: copy one 512-unit path.
em.label('tree_copy_path_entry')
em.mov_r64_r64('rsi','rdx'); em.mov_r64_r64('rdi','rcx'); em.mov_r32_imm('rax',128)
em.label('tcpe_loop'); em.mov_r64_mreg('r10','rsi',0); em.mov_mreg_reg64('rdi',0,'r10'); em.add_r64_imm8('rsi',8); em.add_r64_imm8('rdi',8); em.sub_r32_imm8('rax',1); em.jcc(0x85,'tcpe_loop'); em.emit(0xC3)

# rcx = path -> eax = expanded index, or -1.
em.label('tree_find_expanded_index')
em.emit(0x41,0x54); em.emit(0x41,0x55); em.emit(0x41,0x56); em.emit(0x48,0x83,0xEC,0x30)
em.mov_r64_r64('r12','rcx')
em.mov_r32_ripmem('r13',bsyms['tree_expanded_count']); em.xor32('r14')
em.label('tfei_loop')
em.cmp_r32_r32('r14','r13'); em.jcc(0x8D,'tfei_no')
em.mov_r64_r64('rcx','r12'); em.mov_r32_imm('rdx',0xFFFFFFFF)
em.lea_rip('r8',bsyms['tree_expanded']); em.mov_r32_r32('rax','r14'); em.shl_r32_imm8('rax',10); em.add_r64_r64('r8','rax')
em.mov_r32_imm('r9',0xFFFFFFFF); em.mov_mrsp_imm32(0x20,0); em.call_iat('CompareStringOrdinal'); em.cmp_r32_imm('rax',2); em.jcc(0x84,'tfei_yes')
em.add_r32_imm8('r14',1); em.jmp('tfei_loop')
em.label('tfei_yes'); em.mov_r32_r32('rax','r14'); em.jmp('tfei_ret')
em.label('tfei_no'); em.mov_r32_imm('rax',0xFFFFFFFF)
em.label('tfei_ret'); em.add_r64_imm8('rsp',0x30); em.emit(0x41,0x5E); em.emit(0x41,0x5D); em.emit(0x41,0x5C); em.emit(0xC3)
...
em.lea_rip('rcx',bsyms['tree_expand_path_buf']); em.mov_r64_r64('rdx','r12'); em.call_iat('lstrcpyW')
em.lea_rip('rax',bsyms['tree_expand_path_buf']); em.mov_ripmem_r64(bsyms['tree_expand_path'],'rax')
...
em.mov_ripmem_imm32(bsyms['tree_enum_pass'],0)
...
em.mov_r32_ripmem('rax',bsyms['tree_enum_pass']); em.test32('rax'); em.jcc(0x85,'tree_enum_files')
em.test32('r14'); em.jcc(0x84,'tree_enum_next')
em.jmp('tree_enum_add')
em.label('tree_enum_files'); em.test32('r14'); em.jcc(0x85,'tree_enum_next')
em.label('tree_enum_add')
...
# A tree row owns only TREE_PATH_UNITS WCHARs. The workspace model still keeps
# the full 255-character component, but paths that do not fit the legacy tree
# row are omitted from the UI projection instead of overflowing its metadata.
em.mov_r64_r64('rcx','rax'); em.call_iat('lstrlenW'); em.cmp_r32_imm('rax',TREE_PATH_UNITS); em.jcc(0x83,'tree_enum_next')
em.mov_r32_ripmem('r14',bsyms['tree_insert_flags']); em.test32('r14'); em.jcc(0x84,'tree_enum_insert')
em.mov_r64_ripmem('rcx',bsyms['tree_insert_path']); em.call_label('tree_is_expanded'); em.test32('rax'); em.jcc(0x84,'tree_enum_insert'); em.or_r32_imm('r14',TREE_FLAG_EXPANDED)
...
em.mov_r32_ripmem('rax',bsyms['tree_enum_pass']); em.test32('rax'); em.jcc(0x85,'tree_enum_pass_done')
em.mov_r64_r64('rcx','r15'); em.call_iat('FindClose'); em.mov_ripmem_imm32(bsyms['tree_enum_pass'],1)
em.lea_rip('rcx',bsyms['ws_pattern']); em.lea_rip('rdx',bsyms['ws_find_data']); em.call_iat('FindFirstFileW'); em.cmp_rax_neg1(); em.jcc(0x84,'tree_enum_pass_one_empty'); em.mov_r64_r64('r15','rax'); em.jmp('tree_enum_loop')
em.label('tree_enum_pass_one_empty'); em.mov_r32_imm('rax',1); em.jmp('tree_expand_ret')
em.label('tree_enum_pass_done'); em.mov_r64_r64('rcx','r15'); em.call_iat('FindClose'); em.mov_r32_imm('rax',1); em.jmp('tree_expand_ret')
...
em.lea_rip('rcx',bsyms['tree_root_path']); em.call_iat('lstrlenW'); em.cmp_r32_imm('rax',TREE_PATH_UNITS); em.jcc(0x83,'tree_rebuild_empty')
...
em.label('tree_rebuild_fail'); em.mov_ripmem_imm32(bsyms['tree_row_count'],0); em.xor32('rax'); em.jmp('tree_rebuild_ret')
...
# rcx = directory path: add or remove it from the expanded set, then rebuild
# both the model and the ListBox projection.
em.label('tree_toggle_path')
em.emit(0x41,0x54); em.emit(0x41,0x55); em.emit(0x41,0x56); em.emit(0x41,0x57); em.emit(0x56); em.emit(0x57); em.emit(0x48,0x83,0xEC,0x28)
em.mov_r64_r64('r12','rcx'); em.call_label('tree_find_expanded_index'); em.cmp_r32_imm('rax',0xFFFFFFFF); em.jcc(0x85,'ttp_remove')
em.mov_r32_ripmem('r14',bsyms['tree_expanded_count']); em.cmp_r32_imm('r14',TREE_EXPANDED_MAX); em.jcc(0x83,'ttp_apply')
em.mov_r32_r32('rax','r14'); em.shl_r32_imm8('rax',10); em.lea_rip('rcx',bsyms['tree_expanded']); em.add_r64_r64('rcx','rax'); em.mov_r64_r64('rdx','r12'); em.call_label('tree_copy_path_entry')
em.mov_r32_ripmem('rax',bsyms['tree_expanded_count']); em.add_r32_imm8('rax',1); em.mov_ripmem_r32(bsyms['tree_expanded_count'],'rax'); em.jmp('ttp_apply')
em.label('ttp_remove')
...
# Slice 2: the ListBox is now a projection of the flattened visible tree.
...
# Each ListBox item stores its tree-row index in itemData. The owner-draw code
# and the click handlers both consume that single index.
em.mov_r32_ripmem('r13',bsyms['tree_row_count'])
em.xor32('r12')
em.label('rfl_loop')
...
em.mov_r64_ripmem('rcx',bsyms['hwnd_files']); em.mov_r32_imm('rdx',0x019A); em.mov_r32_ripmem('r8',bsyms['rfl_item_index']); em.mov_r32_r32('r9','r12'); em.call_iat('SendMessageW')
...
# V8.6.4：View 菜单面板项的勾选点。勾选语义 = 互斥最大化：Files 勾选当且仅当
# files_state==0（maximized）且 outline_state==2（minimized），Outline 对称；
# half/half 与标题栏三态产生的其它组合一律不勾选。panel_mode 不再参与勾选，
# 它仍归 set_panel_mode 所有（文件列表键路由与状态栏）。
em.label('sync_panel_menu')
em.emit(0x48,0x83,0xEC,0x28)
em.mov_r64_ripmem('rcx',bsyms['hmenu_view']); em.test64('rcx'); em.jcc(0x84,'spmm_ret')
em.mov_r32_ripmem('rax',bsyms['files_state']); em.test32('rax'); em.jcc(0x85,'spmm_files_off')
em.mov_r32_ripmem('rax',bsyms['outline_state']); em.cmp_r32_imm('rax',2); em.jcc(0x85,'spmm_files_off')
em.mov_r32_imm('r8',0x8); em.jmp('spmm_files_check')
em.label('spmm_files_off'); em.xor32('r8')
em.label('spmm_files_check')
em.mov_r64_ripmem('rcx',bsyms['hmenu_view']); em.mov_r32_imm('rdx',1308); em.call_iat('CheckMenuItem')
em.mov_r32_ripmem('rax',bsyms['outline_state']); em.test32('rax'); em.jcc(0x85,'spmm_outline_off')
em.mov_r32_ripmem('rax',bsyms['files_state']); em.cmp_r32_imm('rax',2); em.jcc(0x85,'spmm_outline_off')
em.mov_r32_imm('r8',0x8); em.jmp('spmm_outline_check')
em.label('spmm_outline_off'); em.xor32('r8')
em.label('spmm_outline_check')
em.mov_r64_ripmem('rcx',bsyms['hmenu_view']); em.mov_r32_imm('rdx',1309); em.call_iat('CheckMenuItem')
em.label('spmm_ret'); em.add_r64_imm8('rsp',0x28); em.emit(0xC3)

# V8.6.4：菜单面板命令与标题栏三态共享 files_state/outline_state 状态机。
# 点击未勾选项 = 排他最大化（自己 maximized、另一 minimized）；再次点击已勾
# 选项 = 恢复默认 half/half。布局应用走 resize_children，勾选走 sync_panel_menu。
em.label('cmd_panel_files')
em.mov_r32_ripmem('rax',bsyms['files_state']); em.test32('rax'); em.jcc(0x85,'cpf_set')
em.mov_r32_ripmem('rax',bsyms['outline_state']); em.cmp_r32_imm('rax',2); em.jcc(0x85,'cpf_set')
em.jmp('cpf_restore')
em.label('cpf_set')
em.mov_ripmem_imm32(bsyms['files_state'],0); em.mov_ripmem_imm32(bsyms['outline_state'],2); em.jmp('cpf_apply')
em.label('cpf_restore')
em.mov_ripmem_imm32(bsyms['files_state'],1); em.mov_ripmem_imm32(bsyms['outline_state'],1)
em.label('cpf_apply')
em.call_label('resize_children'); em.call_label('sync_panel_menu'); em.jmp('msg_loop')
em.label('cmd_panel_outline')
em.mov_r32_ripmem('rax',bsyms['outline_state']); em.test32('rax'); em.jcc(0x85,'cpo_set')
em.mov_r32_ripmem('rax',bsyms['files_state']); em.cmp_r32_imm('rax',2); em.jcc(0x85,'cpo_set')
em.jmp('cpo_restore')
em.label('cpo_set')
em.mov_ripmem_imm32(bsyms['outline_state'],0); em.mov_ripmem_imm32(bsyms['files_state'],2); em.jmp('cpo_apply')
em.label('cpo_restore')
em.mov_ripmem_imm32(bsyms['outline_state'],1); em.mov_ripmem_imm32(bsyms['files_state'],1)
em.label('cpo_apply')
em.call_label('resize_children'); em.call_label('sync_panel_menu'); em.jmp('msg_loop')

# V8.6.4：预留的 AI 右侧栏命令。翻转状态并同步勾选；V8.6 布局不消费该状态，
# V9 的 AI 侧栏布局与标题行按钮将共同消费它。
em.label('cmd_right_sidebar')
em.mov_r32_ripmem('rax',bsyms['right_sidebar_visible']); em.test32('rax'); em.jcc(0x85,'crs_on')
em.mov_r32_imm('rax',1); em.jmp('crs_store')
em.label('crs_on'); em.xor32('rax')
em.label('crs_store'); em.mov_ripmem_r32(bsyms['right_sidebar_visible'],'rax')
em.mov_r64_ripmem('rcx',bsyms['hmenu_view']); em.mov_r32_r32('r8','rax'); em.shl_r32_imm8('r8',3); em.mov_r32_imm('rdx',1315); em.call_iat('CheckMenuItem'); em.jmp('msg_loop')

# V8.6.4：File 菜单最近文件命令（1316-1325）。每个 ID 只装载索引再进共用
# 核心；核心复用文件树打开的整条路径——temp_path ← recent[idx]，置
# open_bypass_picker 后尾跳 destructive_request，与 woe_file 完全同一条
# 未保存保护 → cmd_open_selected 读取/解码/提交链。
for _ri in range(10):
    em.label('cmd_recent_%d' % _ri)
    em.mov_r32_imm('r10', _ri)
    em.jmp('cmd_recent_open')
em.label('cmd_recent_open')
em.mov_r32_ripmem('rax',bsyms['recent_count']); em.cmp_r32_r32('r10','rax'); em.jcc(0x83,'msg_loop')
em.lea_rip('r8',bsyms['recent_paths']); em.mov_r32_r32('rax','r10'); em.shl_r32_imm8('rax',10); em.add_r64_r64('r8','rax')
em.lea_rip('rcx',bsyms['temp_path']); em.mov_r64_r64('rdx','r8'); em.call_iat('lstrcpyW')
...
# V8.6.4：把 current_path 推进最近文件列表（只在 open_commit 后调用）。
# 已在表中则先删旧（去重并提最前），再整体后移一格写入头部；超过 10 条时
# 最旧一条（索引 9）自然被挤出。比较用 CompareStringOrdinal，与
# tree_is_expanded 的路径比较同款。
em.label('push_recent_path')
em.emit(0x41,0x54); em.emit(0x41,0x55); em.emit(0x41,0x56); em.emit(0x41,0x57); em.emit(0x48,0x83,0xEC,0x28)
em.lea_rip('rcx',bsyms['current_path']); em.movzx_eax_word_ptr('rcx'); em.test32('rax'); em.jcc(0x84,'prp_ret')
em.mov_r32_ripmem('r12',bsyms['recent_count'])
em.xor32('r13')
em.label('prp_scan')
em.cmp_r32_r32('r13','r12'); em.jcc(0x83,'prp_shift')
em.lea_rip('r14',bsyms['recent_paths']); em.mov_r32_r32('rax','r13'); em.shl_r32_imm8('rax',10); em.add_r64_r64('r14','rax')
em.lea_rip('rcx',bsyms['current_path']); em.mov_r32_imm('rdx',0xFFFFFFFF); em.mov_r64_r64('r8','r14'); em.mov_r32_imm('r9',0xFFFFFFFF); em.mov_mrsp_imm32(0x20,0); em.call_iat('CompareStringOrdinal')
em.cmp_r32_imm('rax',2); em.jcc(0x84,'prp_found')
em.add_r32_imm8('r13',1); em.jmp('prp_scan')
em.label('prp_found')
em.mov_r32_r32('r14','r13')
em.label('prp_compact')
em.mov_r32_r32('rax','r14'); em.add_r32_imm8('rax',1); em.cmp_r32_r32('rax','r12'); em.jcc(0x8D,'prp_compact_done')
em.lea_rip('rcx',bsyms['recent_paths']); em.mov_r32_r32('rax','r14'); em.shl_r32_imm8('rax',10); em.add_r64_r64('rcx','rax')
em.mov_r32_r32('rax','r14'); em.add_r32_imm8('rax',1); em.shl_r32_imm8('rax',10); em.lea_rip('rdx',bsyms['recent_paths']); em.add_r64_r64('rdx','rax')
em.call_iat('lstrcpyW')
em.add_r32_imm8('r14',1); em.jmp('prp_compact')
em.label('prp_compact_done')
em.sub_r32_imm8('r12',1)
em.label('prp_shift')
em.cmp_r32_imm('r12',10); em.jcc(0x8C,'prp_grow')
em.mov_r32_imm('r12',10); em.jmp('prp_move')
em.label('prp_grow'); em.add_r32_imm8('r12',1)
em.label('prp_move')
em.mov_r32_r32('r14','r12'); em.sub_r32_imm8('r14',1)
em.label('prp_move_loop')
em.test32('r14'); em.jcc(0x84,'prp_write')
em.lea_rip('rcx',bsyms['recent_paths']); em.mov_r32_r32('rax','r14'); em.shl_r32_imm8('rax',10); em.add_r64_r64('rcx','rax')
em.mov_r32_r32('rax','r14'); em.sub_r32_imm8('rax',1); em.shl_r32_imm8('rax',10); em.lea_rip('rdx',bsyms['recent_paths']); em.add_r64_r64('rdx','rax')
em.call_iat('lstrcpyW')
em.sub_r32_imm8('r14',1); em.jmp('prp_move_loop')
em.label('prp_write')
em.lea_rip('rcx',bsyms['recent_paths']); em.lea_rip('rdx',bsyms['current_path']); em.call_iat('lstrcpyW')
em.mov_ripmem_r32(bsyms['recent_count'],'r12')
em.label('prp_ret')
em.add_r64_imm8('rsp',0x28); em.emit(0x41,0x5F); em.emit(0x41,0x5E); em.emit(0x41,0x5D); em.emit(0x41,0x5C); em.emit(0xC3)

# V8.6.4：重建 File 菜单尾部动态段。file_menu_base_count 只数静态命令段
# （New..Close，不含尾部 sep + Exit），先删除基准之后的所有项，再追加
# [sep + 最近文件项（1316..1315+count）+ sep + Exit]；空表时只补 sep + Exit，
# 与静态构建的初始尾部完全一致。菜单项文本直接指向 BSS 中的路径缓冲，
# AppendMenuW 拷贝文本，缓冲之后复用不影响已显示的菜单。
em.label('rebuild_recent_menu')
em.emit(0x41,0x54); em.emit(0x41,0x55); em.emit(0x41,0x56); em.emit(0x41,0x57); em.emit(0x48,0x83,0xEC,0x28)
em.mov_r64_ripmem('rcx',bsyms['hmenu_file']); em.call_iat('GetMenuItemCount'); em.mov_r32_r32('r12','rax')
em.mov_r32_ripmem('r13',bsyms['file_menu_base_count'])
em.label('rrm_trim')
em.cmp_r32_r32('r13','r12'); em.jcc(0x8D,'rrm_append')
em.sub_r32_imm8('r12',1)
em.mov_r64_ripmem('rcx',bsyms['hmenu_file']); em.mov_r32_r32('rdx','r12'); em.mov_r32_imm('r8',0x400); em.call_iat('RemoveMenu')
em.jmp('rrm_trim')
em.label('rrm_append')
em.mov_r64_ripmem('rcx',bsyms['hmenu_file']); em.mov_r32_imm('rdx',0x800); em.xor32('r8'); em.xor32('r9'); em.call_iat('AppendMenuW')
em.mov_r32_ripmem('r12',bsyms['recent_count']); em.test32('r12'); em.jcc(0x84,'rrm_exit')
em.xor32('r13')
em.label('rrm_item')
em.cmp_r32_r32('r13','r12'); em.jcc(0x8D,'rrm_sep2')
em.lea_rip('r9',bsyms['recent_paths']); em.mov_r32_r32('rax','r13'); em.shl_r32_imm8('rax',10); em.add_r64_r64('r9','rax')
em.mov_r64_ripmem('rcx',bsyms['hmenu_file']); em.xor32('rdx'); em.mov_r32_imm('r8',1316); em.add_r32_r32('r8','r13'); em.call_iat('AppendMenuW')
em.add_r32_imm8('r13',1); em.jmp('rrm_item')
em.label('rrm_sep2')
em.mov_r64_ripmem('rcx',bsyms['hmenu_file']); em.mov_r32_imm('rdx',0x800); em.xor32('r8'); em.xor32('r9'); em.call_iat('AppendMenuW')
em.label('rrm_exit')
em.mov_r64_ripmem('rcx',bsyms['hmenu_file']); em.xor32('rdx'); em.mov_r32_imm('r8',1005); em.lea_rip('r9',rsyms['m_exit']); em.call_iat('AppendMenuW')
em.add_r64_imm8('rsp',0x28); em.emit(0x41,0x5F); em.emit(0x41,0x5E); em.emit(0x41,0x5D); em.emit(0x41,0x5C); em.emit(0xC3)
...
em.label('pcs_apply'); em.call_label('resize_children'); em.call_label('sync_panel_menu'); em.add_r64_imm8('rsp',0x28); em.emit(0xC3)
...
em.mov_r32_r32('r12','rax')
em.mov_r64_ripmem('rcx',bsyms['hwnd_files']); em.mov_r32_imm('rdx',0x0199); em.mov_r32_r32('r8','r12'); em.xor32('r9'); em.call_iat('SendMessageW')
em.cmp_r32_imm('rax',0xFFFFFFFF); em.jcc(0x84,'woe_ret')
em.mov_r32_r32('r12','rax'); em.mov_r32_r32('rdx','r12'); em.call_label('tree_row_ptr'); em.mov_r64_r64('r15','rax')
em.mov_r32_mreg('r10','r15',TREE_OFF_FLAGS); em.test32('r10'); em.jcc(0x84,'woe_file')
em.mov_r64_r64('rcx','r15'); em.call_label('tree_toggle_path')
em.mov_r64_ripmem('rcx',bsyms['hwnd_files']); em.call_iat('SetFocus'); em.jmp('woe_ret')
em.label('woe_file')
em.lea_rip('rcx',bsyms['temp_path']); em.mov_r64_r64('rdx','r15'); em.call_iat('lstrcpyW')
em.mov_r64_ripmem('rcx',bsyms['hwnd_files']); em.call_iat('SetFocus')
em.mov_ripmem_imm32(bsyms['open_bypass_picker'],1)
em.mov_ripmem_imm32(bsyms['pending_destructive_action'],2)
...
if OPEN_TEST_BUILD:
    ...
    em.label('cmd_folder_com_probe')
    em.emit(0x48,0x83,0xEC,0x30)
    em.mov_ripmem_imm32(bsyms['fod_probe_entered'],1)
    em.mov_ripmem_imm32(bsyms['fod_ptr'],0); em.mov_ripmem_imm32(bsyms['fod_probe_hresult'],0)
    em.xor32('rcx'); em.mov_r32_imm('rdx',2); em.call_iat('CoInitializeEx'); em.mov_ripmem_r32(bsyms['fod_probe_coinit'],'rax')
    em.lea_rip('rcx',rsyms['clsid_fileopendialog']); em.xor32('rdx'); em.mov_r32_imm('r8',1)
    em.lea_rip('r9',rsyms['iid_ifileopen']); em.lea_rip('rax',bsyms['fod_ptr']); em.mov_mrsp_reg64(0x20,'rax'); em.call_iat('CoCreateInstance')
    em.mov_ripmem_r32(bsyms['fod_probe_hresult'],'rax')
    em.mov_r64_ripmem('rax',bsyms['fod_ptr']); em.test64('rax'); em.jcc(0x84,'folder_probe_done')
    em.mov_r64_mreg('rax','rax',0); em.mov_ripmem_r64(bsyms['fod_probe_vtbl'],'rax')
    em.mov_r64_ripmem('rcx',bsyms['fod_ptr']); em.lea_rip('rdx',bsyms['fod_options']); em.mov_r64_mreg('rax','rcx',0); em.mov_r64_mreg('rax','rax',80); em.call_r64('rax')
    em.mov_ripmem_r32(bsyms['fod_probe_options_hresult'],'rax')
    em.mov_r64_ripmem('rcx',bsyms['fod_ptr']); em.mov_r64_mreg('rax','rcx',0); em.mov_r64_mreg('rax','rax',16); em.call_r64('rax')
    em.label('folder_probe_done'); em.add_r64_imm8('rsp',0x30); em.jmp('msg_loop')
...
em.lea_rip('r11',bsyms['charfmt']); em.mov_mreg_imm32('r11',0,116); em.mov_mreg_imm32('r11',4,0xE0000000); em.mov_mreg_imm32('r11',8,0); em.mov_mreg_imm32('r11',12,255)
...
em.lea_rip('r11',bsyms['charfmt']); em.mov_mreg_imm32('r11',12,255)
...
em.label('theme_del_scroll_hot'); em.mov_r64_ripmem('rcx',bsyms['hbrush_scroll_hot']); em.test64('rcx'); em.jcc(0x84,'theme_del_border'); em.call_iat('DeleteObject')
em.label('theme_del_border'); em.mov_r64_ripmem('rcx',bsyms['hbrush_border']); em.test64('rcx'); em.jcc(0x84,'theme_del_caption_pen'); em.call_iat('DeleteObject')
...
em.mov_r32_imm('rcx',0); em.mov_r32_imm('rdx',1); em.mov_r32_imm('r8',0x00FFFFFF); em.call_iat('CreatePen'); em.mov_ripmem_r64(bsyms['hpen_caption_hot'],'rax')
em.mov_r32_imm('rcx',0x003C3C3C); em.call_iat('CreateSolidBrush'); em.mov_ripmem_r64(bsyms['hbrush_border'],'rax')
em.jmp('theme_controls')
...
em.mov_r32_imm('rcx',0x00D0D0D0); em.call_iat('CreateSolidBrush'); em.mov_ripmem_r64(bsyms['hbrush_border'],'rax')
...
em.mov_r64_ripmem('rcx',bsyms['hwnd_files']); em.mov_r64_ripmem('rdx',bsyms['theme_name_ptr']); em.xor32('r8'); em.call_iat('SetWindowTheme')
...
# Windows 11: explicitly color border/caption/text.  The border colour must never
# match the window background, otherwise the frame is invisible: V8.6.4 shipped
# #1A1A1A on a #202020 backdrop and pure white on the light backdrop, which is
# exactly why the window had no discernible outline.  The dark value is measured
# from a Windows Explorer screenshot (#3C3C3C frame on the same #202020 surface,
# 2 physical pixels at 150% scaling); the light value follows the same idea on
# the #F5F5F5 caption.
em.mov_r32_ripmem('rax',bsyms['theme_dark']); em.test32('rax'); em.jcc(0x84,'theme_dwm_light_colors')
em.mov_ripmem_imm32(bsyms['theme_color'],0x003C3C3C); em.mov_r64_ripmem('rcx',bsyms['hwnd_main']); em.mov_r32_imm('rdx',34); em.lea_rip('r8',bsyms['theme_color']); em.mov_r32_imm('r9',4); em.call_iat('DwmSetWindowAttribute')
...
for _theme_child in ('hwnd_files_header', 'hwnd_outline_header',
                     'hwnd_panel_divider', 'hwnd_outline_gutter'):
    em.mov_r64_ripmem('rcx',bsyms[_theme_child]); em.test64('rcx'); em.jcc(0x84,'theme_skip_'+_theme_child); em.xor32('rdx'); em.mov_r32_imm('r8',1); em.call_iat('InvalidateRect'); em.mov_r64_ripmem('rcx',bsyms[_theme_child]); em.call_iat('UpdateWindow'); em.label('theme_skip_'+_theme_child)
em.mov_r64_r64('rcx','rbx'); em.xor32('rdx'); em.mov_r32_imm('r8',1); em.call_iat('InvalidateRect'); em.mov_r64_r64('rcx','rbx'); em.call_iat('UpdateWindow'); em.call_label('paint_menu_gaps')
...
em.label('exit_brush_scroll_hot'); em.mov_r64_ripmem('rcx',bsyms['hbrush_scroll_hot']); em.test64('rcx'); em.jcc(0x84,'exit_brush_border'); em.call_iat('DeleteObject')
em.label('exit_brush_border'); em.mov_r64_ripmem('rcx',bsyms['hbrush_border']); em.test64('rcx'); em.jcc(0x84,'exit_accel'); em.call_iat('DeleteObject')
...
# V8.6.1：两块滚动面共用同一套绘制。thumb 矩形由 *_scroll_layout 预先算好
# （常态 6px、指针靠近后 11px，都居中于 17px 槽位），绘制阶段只按矩形画一个
# 圆角"胶囊"，和主编辑区右侧的原生滚动条同一种观感：细条 + 悬停变粗。
def emit_thumb_paint(tag, visible_sym, drag_sym, hot_sym, rect_sym):
    em.mov_r32_ripmem('rax',bsyms[visible_sym]); em.test32('rax'); em.jcc(0x84,'sp_endpaint')
    em.mov_r32_ripmem('rax',bsyms[drag_sym]); em.test32('rax'); em.jcc(0x85,'sp_' + tag + '_hot')
    em.mov_r32_ripmem('rax',bsyms[hot_sym]); em.test32('rax'); em.jcc(0x85,'sp_' + tag + '_hot')
    em.mov_r64_ripmem('r8',bsyms['hbrush_scroll_thumb']); em.jmp('sp_' + tag + '_brush')
    em.label('sp_' + tag + '_hot'); em.mov_r64_ripmem('r8',bsyms['hbrush_scroll_hot'])
    em.label('sp_' + tag + '_brush')
    # 刷子进 DC，再用 NULL_PEN(8) 让 RoundRect 只填充不描边。
    em.mov_r64_ripmem('rcx',bsyms['paint_hdc']); em.mov_r64_r64('rdx','r8'); em.call_iat('SelectObject')
    em.mov_r32_imm('rcx',8); em.call_iat('GetStockObject')
    em.mov_r64_ripmem('rcx',bsyms['paint_hdc']); em.mov_r64_r64('rdx','rax'); em.call_iat('SelectObject')
    # RoundRect(hdc, left, top, right, bottom, ellipseW, ellipseH)
    em.lea_rip('rcx',bsyms[rect_sym])
    em.mov_r32_mreg('rdx','rcx',0); em.mov_r32_mreg('r8','rcx',4); em.mov_r32_mreg('r9','rcx',8); em.mov_r32_mreg('rax','rcx',12)
    em.mov_mrsp_reg32(0x20,'rax')
    em.mov_r32_r32('rax','r9'); em.sub_r32_r32('rax','rdx'); em.mov_mrsp_reg32(0x28,'rax'); em.mov_mrsp_reg32(0x30,'rax')
    em.mov_r64_ripmem('rcx',bsyms['paint_hdc']); em.call_iat('RoundRect')
...
emit_thumb_paint('outline', 'outline_scroll_visible', 'outline_scroll_drag', 'outline_scroll_hot', 'outline_thumb_rect')
em.jmp('sp_endpaint')
em.label('sp_state_files')
emit_thumb_paint('files', 'files_scroll_visible', 'files_scroll_drag', 'files_scroll_hot', 'files_thumb_rect')
...
# V8.6.1：主窗口只保留 WS_THICKFRAME 用于边缘缩放。
# V8.6.5：改为保留 1px 非客户区。V8.6.4 让客户区覆盖整个窗口，于是 wp_ncpaint
# 画的四边全都落在客户区最外圈，被铺满客户区的子窗口盖住，边框永远不可见。
# 现在把客户区四边各内缩 1px，边框画在真正属于自己的 1px 非客户区上；子窗口以
# 客户区为原点，会整体内移 1px，布局不需要另行改动。
em.label('wp_nccalcsize')
em.test64('r8'); em.jcc(0x84,'wp_nccalc_false')            # wParam == FALSE -> 默认
em.mov_r32_mreg('rax','r9',0); em.add_r32_imm8('rax',1); em.mov_mreg_reg32('r9',0,'rax')
em.mov_r32_mreg('rax','r9',4); em.add_r32_imm8('rax',1); em.mov_mreg_reg32('r9',4,'rax')
em.mov_r32_mreg('rax','r9',8); em.sub_r32_imm8('rax',1); em.mov_mreg_reg32('r9',8,'rax')
em.mov_r32_mreg('rax','r9',12); em.sub_r32_imm8('rax',1); em.mov_mreg_reg32('r9',12,'rax')
...
em.label('wp_ncpaint')
if CUSTOM_CAPTION:
    # V8.6.5：窗口边框的所有者是 DWM，不是 GDI。实测（注入 DWMWA_BORDER_COLOR
    # 洋红标记）证明 DWM 会在窗口最外 2 个物理像素上覆盖任何 GDI 绘制，所以这里
    # 不再自绘四边，边框颜色交给 apply_theme 的 DwmSetWindowAttribute(34)。
    # WM_NCCALCSIZE 保留的 1px 非客户区，是让 DWM 恢复绘制这条边框的前提。
    em.xor32('rax')
    em.add_r64_imm8('rsp',0x38)
    em.emit(0xC3)
else:
...
em.mov_r32_mreg('rax','r9',4); em.cmp_r32_imm('rax',2); em.jcc(0x84,'wp_draw_status'); em.cmp_r32_imm('rax',4); em.jcc(0x84,'wp_draw_outline'); em.cmp_r32_imm('rax',14); em.jcc(0x84,'wp_draw_outline'); em.cmp_r32_imm('rax',5); em.jcc(0x84,'wp_draw_corner'); em.cmp_r32_imm('rax',6); em.jcc(0x84,'wp_draw_splitter'); em.cmp_r32_imm('rax',7); em.jcc(0x84,'wp_draw_gutter'); em.cmp_r32_imm('rax',16); em.jcc(0x84,'wp_draw_files_header'); em.cmp_r32_imm('rax',17); em.jcc(0x84,'wp_draw_outline_header'); em.cmp_r32_imm('rax',18); em.jcc(0x84,'wp_draw_panel_divider'); em.jmp('wp_default')
...
assert "TREE_OFF_FLAGS" in _panel_draw_src and "bsyms['outline_level']" not in _panel_draw_src, \
    'the file row branch must colour from the tree row, never outline_level'
...
assert "mov_ripmem_imm32(bsyms['pending_destructive_action'],2)" in _nav_open_src and \
       ...
       "call_label('tree_toggle_path')" in _nav_open_src, \
    'the list must toggle directories and route files through the shared Open transaction'
...
#     正式构建里没有可达入口、菜单勾选与面板状态脱钩、选择文件夹后未切换面板。
...
assert "append_imm('r14',0,1308,'m_panel_files')" in _production_source and \
       "append_imm('r14',0,1309,'m_panel_outline')" in _production_source, \
    'View must expose the two panel switches; the checked state is owned by ' \
    'sync_panel_menu, so neither item is pre-checked at creation'
for _panel_cmd in ((1006, 'cmd_open_folder'), (1308, 'cmd_panel_files'),
                   (1309, 'cmd_panel_outline'), (1315, 'cmd_right_sidebar')):
    assert _panel_cmd in _command_routes, '%r must be routed' % (_panel_cmd,)
assert "em.label('spmm_files_off')" in _production_source and \
       "em.label('spmm_outline_off')" in _production_source and \
       "bsyms['files_state']" in _production_source and \
       "bsyms['outline_state']" in _production_source, \
    'the menu check state must follow files_state/outline_state (exclusive maximization)'
assert "em.label('cpf_set')" in _production_source and \
       "em.label('cpo_set')" in _production_source and \
       "em.call_label('resize_children'); em.call_label('sync_panel_menu'); em.jmp('msg_loop')" in _production_source, \
    'both menu items must drive the shared three-state machine and reapply layout'
assert "em.label('pcs_apply'); em.call_label('resize_children'); em.call_label('sync_panel_menu')" in _production_source and \
       "em.label('lbd_frame_apply')" in _production_source, \
    'the header click paths must resync the menu check state too'

# (V+) V8.6.4：右侧栏预留与最近文件列表的所有权断言。要拦截的错误模式：Ctrl+J
#     未进加速键表、1315 未路由、recent 推进不在 open_commit 汇聚点上、菜单
#     命令绕过未保存保护直接读文件、菜单重建后 Exit 丢失。
assert (FVIRTKEY|FCONTROL, 0x4A, 1315) in _accels, \
    'Ctrl+J must open the reserved right sidebar command'
assert "append_imm('r14',0,1315,'m_right_sidebar')" in _production_source, \
    'View must expose the reserved Right Sidebar entry next to Left Sidebar'
assert 'right_sidebar_visible' in bsyms and bss_sizes['right_sidebar_visible'] == 4, \
    'the reserved right sidebar must own one 4-byte state field'
for _ri in range(10):
    assert (1316 + _ri, 'cmd_recent_%d' % _ri) in _command_routes, \
        'recent file command %d must be routed' % (1316 + _ri)
_open_commit_src = _production_source[
    _production_source.index("em.label('open_commit')"):
    _production_source.index("em.label('too_large_close')")]
assert "call_label('push_recent_path')" in _open_commit_src and \
       "call_label('rebuild_recent_menu')" in _open_commit_src, \
    'opening a document must be the only place that pushes the recent list'
for _recent_routine in ('cmd_recent_open', 'push_recent_path', 'rebuild_recent_menu'):
    assert _recent_routine in em.labels, '%s must be emitted' % _recent_routine
_recent_src = _production_source[
    _production_source.index("em.label('cmd_recent_open')"):
    _production_source.index("# V8.6.4：把 current_path 推进最近文件列表")]
assert "call_iat('lstrcpyW')" in _recent_src and \
       "em.mov_ripmem_imm32(bsyms['open_bypass_picker'],1)" in _recent_src and \
       "em.mov_ripmem_imm32(bsyms['pending_destructive_action'],2)" in _recent_src and \
       "em.jmp('destructive_request')" in _recent_src, \
    'recent entries must reuse the tree-open path: temp path, picker bypass, destructive protection'
_rebuild_src = _production_source[
    _production_source.index("em.label('rebuild_recent_menu')"):
    _production_source.index("# Build/test probe for the tree projection")]
assert "call_iat('GetMenuItemCount')" in _rebuild_src and \
       "call_iat('RemoveMenu')" in _rebuild_src and \
       "em.mov_r32_imm('r8',1005); em.lea_rip('r9',rsyms['m_exit'])" in _rebuild_src, \
    'the menu rebuild must trim by position and always re-append Exit'
assert "em.mov_r64_ripmem('rcx',bsyms['hmenu_file']); em.call_iat('GetMenuItemCount'); em.sub_r32_imm8('rax',2); em.mov_ripmem_r32(bsyms['file_menu_base_count'],'rax')" in _production_source, \
    'the static File menu base must exclude the trailing separator + Exit ' \
    'that the rebuild re-appends, or Exit would appear twice'
...
# (Y) V8.6.1 文件夹选择器：新式通用项对话框（IFileOpenDialog）与旧式回落的所有权
#     断言。要拦截的错误模式：GUID 字节序写错导致 CoCreateInstance 报
#     REGDB_E_CLASSNOTREG、vtable 槽位偏移错、丢掉旧式回落，
#     以及最阴的一条——从消息泵 jmp 进入的命令处理器用了非 16 倍数的栈帧，
#     于是 API 调用跑在未对齐的栈上（COM 激活会直接失败，而且看起来像"没注册"）。
assert 'CoCreateInstance' in imports['OLE32.dll'] and \
       'CoTaskMemFree' in imports['OLE32.dll'], \
    'the folder picker needs CoCreateInstance and CoTaskMemFree'
assert rdata[rsyms['clsid_fileopendialog'] - RDATA_RVA:
             rsyms['clsid_fileopendialog'] - RDATA_RVA + 16].hex() == \
    '9c5a1cdc8ae8de4da5a160f82a20aef7', 'CLSID_FileOpenDialog bytes must be exact'
assert rdata[rsyms['iid_ifileopen'] - RDATA_RVA:
             rsyms['iid_ifileopen'] - RDATA_RVA + 16].hex() == \
    '88727cd5add46847be029d969532d960', 'IID_IFileOpenDialog bytes must be exact'
_folder_src = _production_source[
    _production_source.index("em.label('cmd_open_folder')"):
    _production_source.index("em.label('cmd_open_dialog')")]
for _folder_line in ("call_iat('CoCreateInstance')",
                     "em.jcc(0x88,'cmd_open_folder_legacy')",
                     "em.or_r32_imm('rdx',0x60)",
                     "em.mov_r64_mreg('rax','rax',72)",
                     "em.mov_r64_mreg('rax','rax',136)",
                     "em.mov_r64_mreg('rax','rax',24)",
                     "em.mov_r64_mreg('rax','rax',160)",
                     "em.mov_r64_mreg('rax','rax',40)",
                     "call_iat('CoTaskMemFree')",
                     "call_iat('SHBrowseForFolderW')"):
    assert _folder_line in _folder_src, \
        'the modern folder picker lost: %s' % _folder_line
for _vtable in ("em.mov_r64_mreg('rax','rax',9)", "em.mov_r64_mreg('rax','rax',10)"):
    assert _vtable not in _folder_src, \
        'IFileDialog vtable slots must be byte offsets (72/80), not slot numbers'
_frame_re = _re.compile(r"em\.emit\(0x48,0x83,0xEC,0x([0-9A-Fa-f]{2})\)")
for _cmd_label in sorted({_lbl for _cid, _lbl in _command_routes}):
    _idx = next((_i for _i, _ln in enumerate(_scan_lines)
                 if "em.label('%s')" % _cmd_label in _ln), None)
    if _idx is None:
        continue
    for _ln in _scan_lines[_idx + 1:_idx + 5]:
        _m = _frame_re.search(_ln)
        if _m:
            assert int(_m.group(1), 16) % 16 == 0, (
...
# (AE) V8.6.5 窗口边框契约（2026-09-19 实测结论）。
# 边框的所有者是 DWM：向 DWMWA_BORDER_COLOR 注入洋红，窗口最外 2 个物理像素立即
# 变洋红，证明 DWM 会覆盖任何 GDI 绘制。因此边框可见性只取决于两个前提，本段把
# 两者都固定住，任一被改回都会让边框重新消失：
#   1) WM_NCCALCSIZE 把客户区四边各内缩 1px，DWM 才会绘制这条边框；
#   2) DWMWA_BORDER_COLOR(34) 不得等于窗口背景色（深色 #202020 / 浅色 #F5F5F5）。
_ncp_start = _production_source.index("em.label('wp_ncpaint')")
_ncp_end = _production_source.index("em.label('wp_activation')")
_ncpaint_src = _production_source[_ncp_start:_ncp_end]
assert "call_iat('FillRect')" not in _ncpaint_src and \
       "call_iat('GetWindowDC')" not in _ncpaint_src, \
    'wp_ncpaint: 边框归 DWM 所有，禁止用 GDI 自绘四边（会与 DWM 边框冲突出双色边）'
assert "mov_mrsp_imm32(0x38" not in _ncpaint_src, \
    'wp_ncpaint: 禁止写 [rsp+0x38]（WndProc 返回地址）'
for _inset_line in (
        "em.mov_r32_mreg('rax','r9',0); em.add_r32_imm8('rax',1); em.mov_mreg_reg32('r9',0,'rax')",
        "em.mov_r32_mreg('rax','r9',4); em.add_r32_imm8('rax',1); em.mov_mreg_reg32('r9',4,'rax')",
        "em.mov_r32_mreg('rax','r9',8); em.sub_r32_imm8('rax',1); em.mov_mreg_reg32('r9',8,'rax')",
        "em.mov_r32_mreg('rax','r9',12); em.sub_r32_imm8('rax',1); em.mov_mreg_reg32('r9',12,'rax')"):
    assert _inset_line in _nc_src, \
        'wp_nccalcsize: 客户区四边必须各内缩 1px 以留出非客户区，缺少: %s' % _inset_line
for _border_line in (
        "em.mov_ripmem_imm32(bsyms['theme_color'],0x003C3C3C); em.mov_r64_ripmem('rcx',bsyms['hwnd_main']); em.mov_r32_imm('rdx',34)",
        "em.mov_ripmem_imm32(bsyms['theme_color'],0x00B0B0B0); em.mov_r64_ripmem('rcx',bsyms['hwnd_main']); em.mov_r32_imm('rdx',34)"):
    assert _border_line in _production_source, \
        'DWMWA_BORDER_COLOR 必须按主题设置，缺少: %s' % _border_line
_flat_border = ("bsyms['theme_color'],%s); "
                "em.mov_r64_ripmem('rcx',bsyms['hwnd_main']); "
                "em.mov_r32_imm('rdx',34)")
for _flat in ('0x001A1A1A', '0xFFFFFFFF'):
    assert _flat_border % _flat not in _production_source, \
        'DWMWA_BORDER_COLOR 不得与窗口背景同色（%s 会让边框融进背景）' % _flat
...
_output_name = (('pemark_x64_v8_6_5_outline_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                if OUTLINE_ALLOC_INJECTED else
                ('pemark_x64_v8_6_5_style_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                if STYLE_ALLOC_INJECTED else
                ('pemark_x64_v8_6_5_render_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                if RENDER_ALLOC_INJECTED else
                ('pemark_x64_v8_6_5_document_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                if DOCUMENT_ALLOC_INJECTED else
                ('pemark_x64_v8_6_5_wide_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                if WIDE_ALLOC_INJECTED else
                ('pemark_x64_v8_6_5_byte_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                if BYTE_ALLOC_INJECTED else
                ('pemark_x64_v8_6_5_open_read_%s.exe' % OPEN_READ_INJECTION_MODE)
                if OPEN_READ_INJECTION_MODE != 'release' else
                'pemark_x64_v8_6_5_open_transaction_test.exe' if OPEN_TEST_BUILD
                else ('pemark_x64_v8_6_5_write_%s.exe' % WRITE_INJECTION_MODE
                      if INJECTED_BUILD else
                      'pemark_x64_v8_6_5.exe' if _RELEASE_CHANNEL else
