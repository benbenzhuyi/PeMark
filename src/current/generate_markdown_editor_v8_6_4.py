...
_VERSION_LABEL = 'V8.6.4' if _RELEASE_CHANNEL else 'V8.6.4 Candidate'
...
em.label('wp_ncpaint')
if CUSTOM_CAPTION:
    # Phase B: 修复 FillRect RECT 指针错误 + 返回地址覆盖。
    # RECT 写入 BSS draw_rect（lea_rip + mov_mreg_imm32），不用 mov_r64_ripmem。
    # 四边: Top{0,0,W,1} Left{0,0,1,H} Right{W-1,0,W,H} Bottom{0,H-1,W,H}
    em.mov_r64_ripmem('rax',bsyms['hbrush_border']); em.test64('rax')
    em.jcc(0x84,'wp_ncpaint_done')
    em.mov_r64_ripmem('rcx',bsyms['hwnd_main']); em.call_iat('GetWindowDC')
    em.mov_ripmem_r64(bsyms['paint_hdc'],'rax')
    em.test64('rax'); em.jcc(0x84,'wp_ncpaint_done')
    em.lea_rip('rdx',bsyms['draw_rect'])
    em.mov_r32_ripmem('rax',bsyms['client_w'])
    em.mov_mreg_imm32('rdx',0,0); em.mov_mreg_imm32('rdx',4,0)
    em.mov_mreg_reg32('rdx',8,'rax'); em.mov_mreg_imm32('rdx',12,1)
    em.mov_r64_ripmem('rcx',bsyms['paint_hdc']); em.lea_rip('rdx',bsyms['draw_rect'])
    em.mov_r64_ripmem('r8',bsyms['hbrush_border']); em.call_iat('FillRect')
    em.lea_rip('rdx',bsyms['draw_rect'])
    em.mov_r32_ripmem('rax',bsyms['client_h'])
    em.mov_mreg_imm32('rdx',0,0); em.mov_mreg_imm32('rdx',4,0)
    em.mov_mreg_imm32('rdx',8,1); em.mov_mreg_reg32('rdx',12,'rax')
    em.mov_r64_ripmem('rcx',bsyms['paint_hdc']); em.lea_rip('rdx',bsyms['draw_rect'])
    em.mov_r64_ripmem('r8',bsyms['hbrush_border']); em.call_iat('FillRect')
    em.lea_rip('rdx',bsyms['draw_rect'])
    em.mov_r32_ripmem('rax',bsyms['client_w']); em.sub_r32_imm8('rax',1)
    em.mov_mreg_reg32('rdx',0,'rax')
    em.mov_mreg_imm32('rdx',4,0); em.mov_mreg_reg32('rdx',8,'rax')
    em.mov_r32_ripmem('rax',bsyms['client_h']); em.mov_mreg_reg32('rdx',12,'rax')
    em.mov_r64_ripmem('rcx',bsyms['paint_hdc']); em.lea_rip('rdx',bsyms['draw_rect'])
    em.mov_r64_ripmem('r8',bsyms['hbrush_border']); em.call_iat('FillRect')
    em.lea_rip('rdx',bsyms['draw_rect'])
    em.mov_r32_ripmem('rax',bsyms['client_h']); em.sub_r32_imm8('rax',1)
    em.mov_mreg_imm32('rdx',0,0); em.mov_mreg_reg32('rdx',4,'rax')
    em.mov_r32_ripmem('rax',bsyms['client_w']); em.mov_mreg_reg32('rdx',8,'rax')
    em.mov_r32_ripmem('rax',bsyms['client_h']); em.mov_mreg_reg32('rdx',12,'rax')
    em.mov_r64_ripmem('rcx',bsyms['paint_hdc']); em.lea_rip('rdx',bsyms['draw_rect'])
    em.mov_r64_ripmem('r8',bsyms['hbrush_border']); em.call_iat('FillRect')
    em.mov_r64_ripmem('rcx',bsyms['hwnd_main']); em.mov_r64_ripmem('rdx',bsyms['paint_hdc']); em.call_iat('ReleaseDC')
    em.label('wp_ncpaint_done')
    em.xor32('rax')
...
# (AE) V8.6.4 WM_NCPAINT FillRect: RECT 指针必须用 lea_rip（地址），
# 禁止 mov_r64_ripmem（内容），禁止写 [rsp+0x38]（返回地址）。
...
assert "mov_r64_ripmem('rdx',bsyms['draw_rect'])" not in _ncpaint_src, \
    'wp_ncpaint: FillRect RDX 必须用 lea_rip 传 RECT 地址，禁止 mov_r64_ripmem 读 RECT 内容'
assert "mov_mrsp_imm32(0x38" not in _ncpaint_src, \
    'wp_ncpaint: 禁止写 [rsp+0x38]（WndProc 返回地址）'
_fillrect_count = _ncpaint_src.count("call_iat('FillRect')")
_lea_count = _ncpaint_src.count("lea_rip('rdx',bsyms['draw_rect'])")
assert _fillrect_count == 4, \
    'wp_ncpaint: 四边描边必须恰好 4 次 FillRect，实际 %d' % _fillrect_count
assert _lea_count >= _fillrect_count, \
    'wp_ncpaint: 每次 FillRect 前必须有 lea_rip(RDX, draw_rect)，RECT 写入前也必须有；lea=%d fillrect=%d' % (_lea_count, _fillrect_count)
assert "test64('rax'); em.jcc(0x84,'wp_ncpaint_done')" in _ncpaint_src, \
    'wp_ncpaint: GetWindowDC 返回后必须 NULL 检查'
...
_output_name = (('pemark_x64_v8_6_4_outline_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                if OUTLINE_ALLOC_INJECTED else
                ('pemark_x64_v8_6_4_style_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                if STYLE_ALLOC_INJECTED else
                ('pemark_x64_v8_6_4_render_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                if RENDER_ALLOC_INJECTED else
                ('pemark_x64_v8_6_4_document_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                if DOCUMENT_ALLOC_INJECTED else
                ('pemark_x64_v8_6_4_wide_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                if WIDE_ALLOC_INJECTED else
                ('pemark_x64_v8_6_4_byte_alloc_%s.exe' % ARENA_ALLOC_INJECTION_MODE)
                if BYTE_ALLOC_INJECTED else
                ('pemark_x64_v8_6_4_open_read_%s.exe' % OPEN_READ_INJECTION_MODE)
                if OPEN_READ_INJECTION_MODE != 'release' else
                'pemark_x64_v8_6_4_open_transaction_test.exe' if OPEN_TEST_BUILD
                else ('pemark_x64_v8_6_4_write_%s.exe' % WRITE_INJECTION_MODE
                      if INJECTED_BUILD else
                      'pemark_x64_v8_6_4.exe' if _RELEASE_CHANNEL else
