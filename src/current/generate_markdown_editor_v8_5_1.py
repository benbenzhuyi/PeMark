...
TEXT_RVA = 0x1000
# V8.5.1 合并基线：继续使用纯 Direct-PE 生成链，保留扩展后的 text 预算。
...
wstr('title','PeMark x64 V8.5.1 — Direct-PE Markdown Editor')
wstr('empty','')
wstr('menu_file','&File')
wstr('menu_edit','&Edit')
...
wstr('about','PeMark x64 V8.5.1\r\nDirect-PE Markdown Editor\r\n\r\nNative PE32+ and x86-64 machine code generated directly, without a C/C++ compiler, assembler, or linker.\r\n\r\nV8.5.1 combines the unified Markdown/Outline scan and single-writer ViewState with deterministic shutdown, lossless fenced-code rendering, safe word-wrap recreation, viewport-bounded formatting, and one cached Outline scrollbar geometry model.')
...
# (H) Entry point begins with the fixed Win64 stack frame used by the main flow.
...
# (I) dispatch 终结断言（退出崩溃根因的防回退门禁）：dispatch_status_done
...
out = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'bin', _build_channel, 'pemark_x64_v8_5_1.exe'))
...
_map_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'docs', 'PE_MEMORY_MAP_V8_5_1.md'))
