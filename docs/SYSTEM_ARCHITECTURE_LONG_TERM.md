...
每个 arena 必须记录所有者、有效长度、已提交容量、保留容量、元素步长和上限。
增长使用经过溢出检查的几何策略。分配失败必须保持原模型完整。

Parser 在 scratch arena 中生成整套 Semantic/Render/Outline/Map，成功后通过
指针与元数据交换一次性提交。失败时不得把部分结果声明为当前模型。
...
每个实验快照记录 commit、EXE hash、环境、契约版本、工作负载 hash、p50/p95/p99、
失败项、Agent 轮次、人工干预和有效性限制。

## 14. 架构变更管理

修改永久约束、状态所有者、规范坐标、文件格式、平台边界、安全边界、公共行为或
测量方法时必须写 ADR。ADR 包含背景、决定、备选方案、后果和回滚路径。
