...
**运行前请关闭所有正在运行的 PeMark 窗口**（包括任务栏里已打开的那个）。EXE 被
占用时无法重写，脚本会检测到并直接给出这条提示，而不是让后续步骤连环失败。安全
软件偶尔会短暂锁定新文件，稍等片刻重跑即可。

节权限、栈回溯这两个套件在验证模式下**不会改写发布二进制**：它们把符号表从
`bin/verify_scratch/` 的临时副本加载出来，只读取与启动 `bin/current` 里的发布
文件。该目录已被 `.gitignore` 忽略。

GUI 冒烟里的关闭步骤会等待最多 15 秒并持续处理"未保存内容"提示对话框。如果
超时，报告里会附带"待处理对话框=N"：N>0 表示程序正在等待回答（测试未及时点到），
N=0 才是真正没有退出的信号。这是为了在出问题时能一眼区分两种情况。
