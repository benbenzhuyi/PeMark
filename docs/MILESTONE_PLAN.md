...
1. **WorkspaceModel 与目录枚举（无 UI）**
   - 状态：根目录、当前目录、条目表（名称 / 属性 / 大小 / 时间戳）
   - `FindFirstFileW` / `FindNextFileW` 枚举，过滤出子目录与 `.md` / `.markdown` /
     `.txt`；排序为目录在前，各自按名称升序
   - 验收：测试命令对 fixture 目录输出条目列表；空目录、拒绝访问、Unicode 名称、
     名称过长都有明确结果
2. **侧边栏面板与文件列表控件**
   - 左侧栏增加"大纲 / 文件"两种模式，复用同一块布局区域与现有滚动条几何
   - 行绘制区分目录与文件
   - 验收：模式切换后布局、绘制与主题正确；列表可滚动可选择；原大纲行为不变
3. **进入目录、返回上级、打开文件**
   - 双击目录进入；Backspace 或菜单返回上级；状态栏显示当前路径
   - 双击文件走**现有** Open 事务路径（`temp_path` + 未保存保护），不新增第二条
     打开逻辑
   - 验收：脏文档先出现保存/放弃/取消；打开后 Source 内容与编码正确；失败保持原文档
4. **菜单与快捷键集成**
