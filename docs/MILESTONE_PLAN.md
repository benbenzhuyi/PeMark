...
用户要求文件界面全面参考 `benbenzhuyi/rabbit-editor` 的实现。据此撤销本节原来
的两个推迟决定——"只做单层列表"与"第一版只读"——文件面板改为可展开目录树，
并补齐新建 / 重命名 / 删除 / 刷新 / 全部折叠等文件操作。完整设计、参考行为清单
与切片划分见 [FILE_PANEL_REDESIGN.md](FILE_PANEL_REDESIGN.md)。

同一修订还取代了本节切片 2 的"两个模式复用同一个 ListBox"：Rabbit 的左边栏是
**文件浏览器在上、大纲导航在下**的双面板，中间有 4px 可拖动分界线，两个标题栏
各自可点击在 half / minimized / maximized 三态之间切换。PeMark 改为两个列表各配
一套 gutter 与滚动条；大纲的三张表与滚动状态保留，只不再与文件共用控件。

仍然保留的取向：不引入 `SysTreeView32`（改用自绘"扁平化可见行"树）、不做
`ReadDirectoryChangesW` 常驻监视、不做窗口几何持久化、始终保持单一可写文档。

### 设计取向（以及为什么不做某些事）

- **先做单层文件列表，不做 TreeView。**（2026-09-16 撤销，见上方范围修订；
  以下为当时的判断与理由，保留以便追溯。）工程里已有自绘 ListBox（大纲）、自绘
  滚动条几何、布局管理器与主题，这三样可整体复用；引入 `SysTreeView32` 需要新的
  控件类、`WM_NOTIFY` 处理与另一套主题适配。核心用途是"找到并打开文件"，单层列表
  加"进入目录 / 返回上级"就够用，层次导航等出现真实需求再考虑。
- **第一版只读。**（2026-09-16 撤销，见上方范围修订；以下为当时的判断与理由。）
  创建、重命名、删除都会破坏用户文件，收益低而验证成本高，推迟到有明确需求时
  单独提案。
...
2. **文件面板重设计（替代原"slices 2–4 已完成"的后续）**
   - 按 [FILE_PANEL_REDESIGN.md](FILE_PANEL_REDESIGN.md) 的切片 0–5 执行：
     ...
     展开集合 → 树绘制与命中测试 → 文件操作（重命名/新建/删除/复制路径）→
     标题栏按钮、刷新、全部折叠、默认根目录 → 键盘导航与选择持久化；
   - 切片 0–5 全部完成：双面板骨架与三态标题栏、树模型与展开集合、树投影与命中
     测试、文件操作（内联重命名、右键菜单、新建去重、回收站删除、复制路径）、
     标题栏根路径缩略与"全部折叠 / 刷新"按钮（含 File 菜单入口、刷新保持选中、
     启动默认加载系统"文档"目录）、键盘导航（`Enter` 展开/打开、`Backspace`
     返回上级、打开后焦点留在面板）与跨刷新/展开折叠的选中持久化。
   - 该面板的底座（目录枚举、面板模式、共用滚动条与主题、Open 事务入口、
     状态栏路径段）在切片过程中持续复用，未出现第二套实现。
   - 每个切片独立可验证、可回滚，验收标准见该文档 §9。
...
- ~~create / rename / delete 等破坏性文件操作~~（2026-09-16 转入
  [FILE_PANEL_REDESIGN.md](FILE_PANEL_REDESIGN.md) 切片 C，删除改为进回收站）；
- ~~树形层次导航~~（2026-09-16 转为自绘扁平树，见同一文档切片 A/B；
  `SysTreeView32` 仍然不做）；
- 拖拽打开；
