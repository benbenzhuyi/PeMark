...
def checks(ns):
    ...
    if 'set_view_mode' in ns['em'].labels:
        def view():
            m=Machine(ns)
            for i in range(20):
                mode=i%2; m.run('set_view_mode',rcx=mode)
                assert m.get('view_mode')==mode
    ...
    if 'scroll_layout' in ns['em'].labels:
        def wrap_preview():
            m=Machine(ns); m.put('view_mode',1); m.put('render_len',123)
            ...
            assert creates[0][2]==m.base+ns['rsyms']['empty'],'large text passed during EDIT creation'
            assert m.create_instances==[1],m.create_instances
            limit=next(i for i,(n,a) in enumerate(m.calls) if n=='SendMessageW' and a[1]==0xc5)
            textload=next(i for i,(n,a) in enumerate(m.calls) if n=='SetWindowTextW' and a[0]==1)
        ...
        def viewport():
            m=Machine(ns); m.put('view_top_pos',1000); m.put('render_len',100000)
...
if __name__=='__main__':
    path=Path(sys.argv[1]) if len(sys.argv)>1 else ROOT/'src/stabilization/generate_markdown_editor_v8_4_24.py'
