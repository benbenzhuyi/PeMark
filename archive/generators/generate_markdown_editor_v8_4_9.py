...
wstr('title','Direct PE Markdown Editor x64 V8.4.9 — No Compiler')
wstr('empty','')
wstr('menu_file','&File')
wstr('menu_edit','&Edit')
...
imports = {
    ...
    'USER32.dll': [
        ...
        'GetCursorPos','ScreenToClient','SetCapture','ReleaseCapture','SetCursor','SetScrollRange','SetScrollPos',
...
em.xor32('rcx'); em.lea_rip('rdx',rsyms['class_listbox']); em.lea_rip('r8',rsyms['empty']); em.mov_r32_imm('r9',0x50010151)
...
# Dedicated splitter child. It owns the separator pixels, so repeated sidebar
# show/hide never leaves stale fragments in the parent client area.
em.xor32('rcx'); em.lea_rip('rdx',rsyms['class_static']); em.lea_rip('r8',rsyms['empty']); em.mov_r32_imm('r9',0x50000100)
em.mov_mrsp_imm32(0x20,228); em.mov_mrsp_imm32(0x28,0); em.mov_mrsp_imm32(0x30,5); em.mov_mrsp_imm32(0x38,590)
...
# Narrow standard scrollbar floats just inside the Outline edge only while the
# pointer is near the splitter. It provides fast thumb dragging without a
# permanently bulky scrollbar between Outline and document.
em.xor32('rcx'); em.lea_rip('rdx',rsyms['class_scrollbar']); em.lea_rip('r8',rsyms['empty']); em.mov_r32_imm('r9',0x40000001)
em.mov_mrsp_imm32(0x20,216); em.mov_mrsp_imm32(0x28,0); em.mov_mrsp_imm32(0x30,10); em.mov_mrsp_imm32(0x38,590)
...
em.mov_rax_mr12(0); em.mov_r64_ripmem('r10',bsyms['hwnd_splitter']); em.cmp_r64_r64('rax','r10'); em.jcc(0x85,'dispatch')
em.mov_ripmem_imm32(bsyms['splitter_drag'],1); em.mov_r64_ripmem('rcx',bsyms['hwnd_splitter']); em.call_iat('SetCapture')
...
em.mov_ripmem_r32(bsyms['outline_scroll_top'],'r8'); em.mov_r64_ripmem('rcx',bsyms['hwnd_outline']); em.mov_r32_imm('rdx',0x0197); em.xor32('r9'); em.call_iat('SendMessageW'); em.mov_r64_ripmem('rcx',bsyms['hwnd_outline_scroll']); em.mov_r32_imm('rdx',2); em.mov_r32_ripmem('r8',bsyms['outline_scroll_top']); em.mov_r32_imm('r9',1); em.call_iat('SetScrollPos'); em.jmp('msg_loop')
...
em.mov_r64_ripmem('rcx',bsyms['hwnd_preview']); em.call_label('capture_surface_state')
em.mov_ripmem_imm32(bsyms['preview_visible_format_only'],1); em.mov_ripmem_imm32(bsyms['preview_theme_dirty'],1)
...
# If a theme switch left offscreen Preview ranges stale, refresh only newly visible
# content after user navigation/scroll input. This keeps theme switching responsive.
...
em.mov_eax_mr12(8); em.cmp_r32_imm('rax',0x0115); em.jcc(0x84,'dispatch_theme_refresh'); em.cmp_r32_imm('rax',0x020A); em.jcc(0x84,'dispatch_theme_refresh'); em.cmp_r32_imm('rax',0x0100); em.jcc(0x85,'dispatch_theme_refresh_done')
em.label('dispatch_theme_refresh'); em.call_label('refresh_preview_visible_theme')
...
# Helper: reveal the narrow Outline thumb only while pointer is near the divider.
...
em.mov_r32_ripmem('r10',bsyms['cursor_pt']); em.mov_r32_ripmem('r11',bsyms['outline_width']); em.mov_r32_r32('rax','r11'); em.sub_r32_imm8('rax',22); em.cmp_r32_r32('r10','rax'); em.jcc(0x8C,'hover_hide'); em.add_r32_imm8('r11',8); em.cmp_r32_r32('r10','r11'); em.jcc(0x8F,'hover_hide')
em.mov_r32_ripmem('r10',bsyms['cursor_pt']+4); em.test32('r10'); em.jcc(0x88,'hover_hide'); em.mov_r32_ripmem('r11',bsyms['client_h']); em.mov_r32_ripmem('rax',bsyms['status_h']); em.sub_r32_r32('r11','rax'); em.cmp_r32_r32('r10','r11'); em.jcc(0x8D,'hover_hide')
em.mov_r32_ripmem('rax',bsyms['outline_scroll_visible']); em.test32('rax'); em.jcc(0x85,'hover_cursor')
em.call_label('sync_outline_scrollbar'); em.mov_r64_ripmem('rcx',bsyms['hwnd_outline_scroll']); em.mov_r32_imm('rdx',5); em.call_iat('ShowWindow'); em.mov_ripmem_imm32(bsyms['outline_scroll_visible'],1)
em.label('hover_cursor')
# Size-WE cursor only on the splitter itself; scrollbar remains normal arrow.
em.lea_rip('rcx',bsyms['cursor_pt']); em.call_iat('GetCursorPos'); em.mov_r64_ripmem('rcx',bsyms['hwnd_splitter']); em.lea_rip('rdx',bsyms['cursor_pt']); em.call_iat('ScreenToClient'); em.mov_r32_ripmem('r10',bsyms['cursor_pt']); em.test32('r10'); em.jcc(0x88,'hover_done'); em.cmp_r32_imm('r10',5); em.jcc(0x83,'hover_done'); em.xor32('rcx'); em.mov_r32_imm('rdx',32644); em.call_iat('LoadCursorW'); em.mov_r64_r64('rcx','rax'); em.call_iat('SetCursor'); em.jmp('hover_done')
em.label('hover_hide'); em.mov_r32_ripmem('rax',bsyms['outline_scroll_visible']); em.test32('rax'); em.jcc(0x84,'hover_done'); em.mov_r64_ripmem('rcx',bsyms['hwnd_outline_scroll']); em.xor32('rdx'); em.call_iat('ShowWindow'); em.mov_ripmem_imm32(bsyms['outline_scroll_visible'],0)
em.label('hover_done'); em.add_r64_imm8('rsp',0x28); em.emit(0xC3)
...
# Helper: after a theme change, lazily repaint only the current Preview viewport.
...
em.mov_r64_ripmem('rcx',bsyms['hwnd_preview']); em.call_label('capture_surface_state'); em.mov_ripmem_imm32(bsyms['preview_visible_format_only'],1); em.call_label('apply_preview_base'); em.call_label('apply_styles'); em.mov_ripmem_imm32(bsyms['preview_visible_format_only'],0); em.mov_r64_ripmem('rcx',bsyms['hwnd_preview']); em.call_label('restore_surface_state')
...
# Cache x/width of the document surface. Outline width is user-resizable; a
# dedicated 5px splitter owns all separator pixels so no stale bright fragments remain.
...
em.mov_r32_ripmem('r10',bsyms['outline_width']); em.mov_r32_r32('r11','r10'); em.add_r32_imm8('r11',5); em.mov_ripmem_r32(bsyms['content_x'],'r11')
...
em.xor32('rdx'); em.xor32('r8'); em.mov_r32_ripmem('r9',bsyms['outline_width']); em.mov_mrsp_reg32(0x20,'r11'); em.mov_mrsp_imm32(0x28,1,qword=True); em.call_iat('MoveWindow')
em.label('resize_splitter')
em.mov_r64_ripmem('rcx',bsyms['hwnd_splitter']); em.test64('rcx'); em.jcc(0x84,'resize_float_scroll')
em.mov_r32_ripmem('rdx',bsyms['outline_width']); em.xor32('r8'); em.mov_r32_imm('r9',5); em.mov_mrsp_reg32(0x20,'r11'); em.mov_mrsp_imm32(0x28,1,qword=True); em.call_iat('MoveWindow')
em.label('resize_float_scroll')
em.mov_r64_ripmem('rcx',bsyms['hwnd_outline_scroll']); em.test64('rcx'); em.jcc(0x84,'resize_doc')
em.mov_r32_ripmem('rdx',bsyms['outline_width']); em.sub_r32_imm8('rdx',12); em.xor32('r8'); em.mov_r32_imm('r9',10); em.mov_mrsp_reg32(0x20,'r11'); em.mov_mrsp_imm32(0x28,1,qword=True); em.call_iat('MoveWindow'); em.jmp('resize_doc')
...
em.mov_r64_ripmem('rax',bsyms['fn_allowdark']); em.mov_r64_ripmem('rcx',bsyms['hwnd_outline_scroll']); em.mov_r32_ripmem('rdx',bsyms['theme_dark']); em.call_r64('rax')
...
em.mov_r64_ripmem('rcx',bsyms['hwnd_outline_scroll']); em.mov_r64_ripmem('rdx',bsyms['theme_name_ptr']); em.xor32('r8'); em.call_iat('SetWindowTheme')
...
em.mov_r32_mreg('rax','r9',4); em.cmp_r32_imm('rax',2); em.jcc(0x84,'wp_draw_status'); em.cmp_r32_imm('rax',4); em.jcc(0x84,'wp_draw_outline'); em.cmp_r32_imm('rax',5); em.jcc(0x84,'wp_draw_corner'); em.jmp('wp_default')
...
out = '/mnt/data/direct_pe_markdown_editor_x64_v8_4_9.exe'
