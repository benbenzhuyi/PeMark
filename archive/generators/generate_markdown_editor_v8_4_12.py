...
wstr('title','Direct PE Markdown Editor x64 V8.4.12 — No Compiler')
wstr('empty','')
wstr('menu_file','&File')
wstr('menu_edit','&Edit')
...
em.xor32('rcx'); em.lea_rip('rdx',rsyms['class_static']); em.lea_rip('r8',rsyms['empty']); em.mov_r32_imm('r9',0x5400010D)  # SS_OWNERDRAW | SS_NOTIFY | WS_CLIPSIBLINGS
...
# V8.4.12: no separate floating SCROLLBAR child. The Outline's native themed
...
# dedicated 4px splitter owns all separator pixels so no stale bright fragments remain.
...
out = '/mnt/data/direct_pe_markdown_editor_x64_v8_4_12.exe'
