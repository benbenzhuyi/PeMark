...
wstr('title','Direct PE Markdown Editor x64 V8.4.15 — No Compiler')
wstr('empty','')
wstr('menu_file','&File')
wstr('menu_edit','&Edit')
...
# V8.4.15 deliberately does not crop or overlay the ListBox non-client scrollbar.
...
# V8.4.15: no scrollbar overlay child. The Outline's own themed ListBox scrollbar
...
# is near the divider. V8.4.15 leaves this non-client scrollbar completely native:
...
# ShowScrollBar can instantiate/recreate the ListBox non-client scrollbar after the
# child theme was originally applied. Re-assert the current Explorer/DarkMode_Explorer
# theme *after* revealing it, then force a non-client frame refresh so the first frame
# never flashes the classic Win32 scrollbar.
...
out = '/mnt/data/direct_pe_markdown_editor_x64_v8_4_15.exe'
