...
wstr('title','Direct PE Markdown Editor x64 V8.4.23 — No Compiler')
wstr('empty','')
wstr('menu_file','&File')
wstr('menu_edit','&Edit')
...
# preview_flag is the model state, but the actually visible surface is the final
# authority. If an earlier load/resize ever desynchronizes them, repair it here so
# the FIRST Ctrl+Shift+P always performs a visible mode switch.
em.mov_r32_ripmem('rax',bsyms['preview_flag']); em.test32('rax'); em.jcc(0x84,'preview_show')
em.mov_r64_ripmem('rcx',bsyms['hwnd_preview']); em.call_iat('IsWindowVisible'); em.test32('rax'); em.jcc(0x85,'preview_hide_active')
em.mov_ripmem_imm32(bsyms['preview_flag'],0); em.jmp('preview_show')
...
# Defensive state reconciliation: only route navigation to Preview if that surface
# is actually visible. This prevents a stale logical flag from sending jumps into
# a hidden RichEdit while Source is on screen.
em.mov_r64_ripmem('rcx',bsyms['hwnd_preview']); em.call_iat('IsWindowVisible'); em.test32('rax'); em.jcc(0x84,'navigate_source')
...
out = '/mnt/data/direct_pe_markdown_editor_x64_v8_4_23.exe'
