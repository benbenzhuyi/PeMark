...
FILE_ALIGN = 0x200
SECT_ALIGN = 0x1000
STYLE_CAP = 131072  # V8.4.17: large-doc Markdown style span capacity
...
wstr('title','Direct PE Markdown Editor x64 V8.4.17 — No Compiler')
wstr('empty','')
wstr('menu_file','&File')
wstr('menu_edit','&Edit')
...
# V8.4.17 deliberately does not crop or overlay the ListBox non-client scrollbar.
# Windows owns its complete themed geometry and hit-testing; stability wins over
# trying to erase a 1-2px visual difference versus RichEdit.
...
# V8.4.17: no scrollbar overlay child. The Outline's own themed ListBox scrollbar
# is the sole scrollbar surface, so thumb painting, hit testing and dragging are all
# handled by Windows without a competing sibling window or Z-order race.
...
# If the native Outline scrollbar is showing, its full non-client strip remains click/drag owned by Windows.
...
# is near the divider. V8.4.17 leaves this non-client scrollbar completely native:
# no trim/overlay window is allowed to cover its track or thumb.
...
# Show the complete native ListBox scrollbar only once.
# V8.4.17 suppresses drawing while the non-client scrollbar is being materialized.
# On some Win11 builds ShowScrollBar paints one classic frame before the theme
# machinery catches up after a Light/Dark switch.  Freeze redraw, reveal it,
# reset+reapply the desired Explorer theme, broadcast WM_THEMECHANGED, force a
# frame recalculation, then enable redraw and paint exactly once.
em.mov_r32_ripmem('rax',bsyms['outline_scroll_visible']); em.test32('rax'); em.jcc(0x85,'hover_cursor')
em.mov_r64_ripmem('rcx',bsyms['hwnd_outline']); em.mov_r32_imm('rdx',0x000B); em.xor32('r8'); em.xor32('r9'); em.call_iat('SendMessageW')
...
# Reset any stale per-window theme cache before applying the selected mode.
em.mov_r64_ripmem('rcx',bsyms['hwnd_outline']); em.xor32('rdx'); em.xor32('r8'); em.call_iat('SetWindowTheme')
...
em.mov_r64_ripmem('rax',bsyms['fn_allowdark']); em.test64('rax'); em.jcc(0x84,'hover_theme_allow_done'); em.mov_r64_ripmem('rcx',bsyms['hwnd_outline']); em.mov_r32_ripmem('rdx',bsyms['theme_dark']); em.call_r64('rax')
em.label('hover_theme_allow_done')
# WM_THEMECHANGED = 0x031A.  Sending it explicitly makes the first non-client
# scrollbar paint use the new theme instead of waiting for the first thumb drag.
em.mov_r64_ripmem('rcx',bsyms['hwnd_outline']); em.mov_r32_imm('rdx',0x031A); em.xor32('r8'); em.xor32('r9'); em.call_iat('SendMessageW')
em.mov_r64_ripmem('rcx',bsyms['hwnd_outline']); em.xor32('rdx'); em.xor32('r8'); em.xor32('r9'); em.mov_mrsp_imm32(0x20,0); em.mov_mrsp_imm32(0x28,0); em.mov_mrsp_imm32(0x30,0x0037); em.call_iat('SetWindowPos')
...
em.mov_r64_ripmem('rcx',bsyms['hwnd_outline']); em.xor32('rdx'); em.xor32('r8'); em.mov_r32_imm('r9',0x0501); em.call_iat('RedrawWindow')
em.mov_r64_ripmem('rcx',bsyms['hwnd_outline']); em.call_iat('UpdateWindow')
...
# Do not auto-hide a visible native scrollbar while the left button is held.
# This covers thumb tracking even when Windows has non-client capture and the
# pointer temporarily leaves the narrow hover zone.
...
em.mov_r64_ripmem('rcx',bsyms['hwnd_splitter']); em.test64('rcx'); em.jcc(0x84,'resize_doc')
...
em.mov_r64_ripmem('rcx',bsyms['hwnd_outline']); em.mov_r64_ripmem('rdx',bsyms['theme_name_ptr']); em.xor32('r8'); em.call_iat('SetWindowTheme')
# If the native Outline scrollbar is already visible during a theme toggle,
# refresh its non-client theme immediately instead of waiting for thumb input.
em.mov_r32_ripmem('rax',bsyms['outline_scroll_visible']); em.test32('rax'); em.jcc(0x84,'theme_outline_scroll_done')
em.mov_r64_ripmem('rcx',bsyms['hwnd_outline']); em.mov_r32_imm('rdx',0x031A); em.xor32('r8'); em.xor32('r9'); em.call_iat('SendMessageW')
em.mov_r64_ripmem('rcx',bsyms['hwnd_outline']); em.xor32('rdx'); em.xor32('r8'); em.xor32('r9'); em.mov_mrsp_imm32(0x20,0); em.mov_mrsp_imm32(0x28,0); em.mov_mrsp_imm32(0x30,0x0037); em.call_iat('SetWindowPos')
em.mov_r64_ripmem('rcx',bsyms['hwnd_outline']); em.xor32('rdx'); em.xor32('r8'); em.mov_r32_imm('r9',0x0501); em.call_iat('RedrawWindow')
em.label('theme_outline_scroll_done')
...
em.mov_r32_mreg('rax','r9',4); em.cmp_r32_imm('rax',2); em.jcc(0x84,'wp_draw_status'); em.cmp_r32_imm('rax',4); em.jcc(0x84,'wp_draw_outline'); em.cmp_r32_imm('rax',5); em.jcc(0x84,'wp_draw_corner'); em.cmp_r32_imm('rax',6); em.jcc(0x84,'wp_draw_splitter'); em.jmp('wp_default')
...
# V8.4.17: reserve a hard text-safe strip for the native non-client scrollbar.
# Themed ListBox scrollbars on some Win11 builds overlay the client edge instead
# of shrinking owner-draw item rectangles immediately.  Clamp DrawText's right
# edge to the current client width, and while the scrollbar is visible reserve
# one full SM_CXVSCROLL width plus 4px.  Outline glyphs can therefore never be
# drawn underneath / through the scrollbar track.
...
em.mov_r32_ripmem('rax',bsyms['outline_scroll_visible']); em.test32('rax'); em.jcc(0x84,'outline_draw_safe_right_ready')
em.mov_r32_ripmem('r11',bsyms['scrollbar_w']); em.sub_r32_r32('r10','r11')
em.label('outline_draw_safe_right_ready')
em.lea_rip('rcx',bsyms['draw_rect']); em.mov_r32_mreg('r11','rcx',8); em.cmp_r32_r32('r11','r10'); em.jcc(0x86,'outline_draw_right_clamped'); em.mov_mreg_reg32('rcx',8,'r10')
em.label('outline_draw_right_clamped')
...
out = '/mnt/data/direct_pe_markdown_editor_x64_v8_4_17.exe'
