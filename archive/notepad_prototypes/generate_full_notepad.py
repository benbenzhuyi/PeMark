...
imports = {
    'KERNEL32.dll': [
        ...
        'MultiByteToWideChar','WideCharToMultiByte','lstrcpyW'
    ],
    'USER32.dll': [
...
# Parent STATIC top-level window
em.xor32('rcx'); em.lea_rip('rdx',rsyms['class_static']); em.lea_rip('r8',rsyms['title']); em.mov_r32_imm('r9',0x10CF0000)
...
em.mov_mrsp_imm32(0x40,0,qword=True); em.mov_mrsp_reg64(0x48,'rdi'); em.mov_mrsp_imm32(0x50,0,qword=True); em.mov_mrsp_imm32(0x58,0,qword=True)
...
em.mov_mrsp_reg64(0x40,'rbx'); em.mov_mrsp_imm32(0x48,1,qword=True); em.mov_mrsp_imm32(0x50,0,qword=True); em.mov_mrsp_imm32(0x58,0,qword=True)
...
em.mov_eax_mr12(8); em.cmp_r32_imm('rax',0x111); em.jcc(0x84,'command')
em.cmp_r32_imm('rax',5); em.jcc(0x84,'on_size')
em.jmp('dispatch')
...
# ---------------- PE writer ----------------
text_raw = bytes(text) + b'\0'*(align(len(text),FILE_ALIGN)-len(text))
rdata_raw = bytes(rdata) + b'\0'*(align(len(rdata),FILE_ALIGN)-len(rdata))
idata_raw = bytes(idata) + b'\0'*(align(len(idata),FILE_ALIGN)-len(idata))
headers_size = 0x400
text_ptr = headers_size
rdata_ptr = text_ptr + len(text_raw)
idata_ptr = rdata_ptr + len(rdata_raw)
...
# tiny DOS message stub data (not executed on Windows NT)
dosmsg=b'This program cannot be run in DOS mode.\r\r\n$'
hdr[0x40:0x40+len(dosmsg)] = dosmsg
p=0x80
hdr[p:p+4]=b'PE\0\0'; p+=4
# COFF
struct.pack_into('<HHIIIHH',hdr,p,0x8664,4,0,0,0,0xF0,0x0022); p+=20
# Optional PE32+
opt=bytearray(b'\0'*0xF0)
struct.pack_into('<HBBIII',opt,0,0x20B,14,0,len(text_raw),len(rdata_raw)+len(idata_raw),BSS_VSIZE)
...
def sh(name,vsize,rva,rawsize,rawptr,chars):
    n=name.encode('ascii')[:8].ljust(8,b'\0')
    return n+struct.pack('<IIIIIIHHI',vsize,rva,rawsize,rawptr,0,0,0,0,chars)

sections=[
    sh('.text',len(text),TEXT_RVA,len(text_raw),text_ptr,0x60000020),
    sh('.rdata',len(rdata),RDATA_RVA,len(rdata_raw),rdata_ptr,0x40000040),
    sh('.idata',len(idata),IDATA_RVA,len(idata_raw),idata_ptr,0xC0000040),
    sh('.bss',BSS_VSIZE,BSS_RVA,0,0,0xC0000080),
]
for s in sections:
    hdr[p:p+40]=s; p+=40

out='/mnt/data/direct_pe_notepad_full_x64.exe'
with open(out,'wb') as f:
    f.write(hdr); f.write(text_raw); f.write(rdata_raw); f.write(idata_raw)

sha=hashlib.sha256(open(out,'rb').read()).hexdigest()
print(out)
print('size',os.path.getsize(out),'text',len(text),'rdata',len(rdata),'idata',len(idata),'bss_vsize',BSS_VSIZE)
print('sha256',sha)
